...
class Base
{
	...
	/**
	 * @var \Borhan\Client\Configuration
	 */
	protected $config;
	...
	/**
	 * @var array<\Borhan\Client\ServiceActionCall>
	 */
	 ...
	 *
	 * @param \Borhan\Client\Configuration $config
	 */
	...
	public function doQueue()
	{
		...
		else
		{
			...
			if ($this->config->getFormat() != self::BORHAN_SERVICE_FORMAT_XML)
			{
				$this->resetRequest();
	...
	private function doCurl($url, $params = array(), $files = array())
	{
		...
		if($this->config->getFormat() == self::BORHAN_SERVICE_FORMAT_JSON)
		{
	...
	public function setClientConfiguration(\Borhan\Client\Type\ClientConfiguration $configuration)
	{
		$params = get_class_vars('\Borhan\Client\Type\ClientConfiguration');
	...
	public function setRequestConfiguration(\Borhan\Client\Type\RequestConfiguration $configuration)
	{
		$params = get_class_vars('\Borhan\Client\Type\RequestConfiguration');
		foreach($params as $param)
		{
			if(is_null($configuration->$param))
			{
	...
	public function checkIfError(\SimpleXMLElement $xml, $throwException = true)
	{
		if(($xml->error) && (count($xml->children()) == 1))
		{
			...
			$arguments = ParseUtils::unmarshalArray($xml->error->args, 'BorhanApiExceptionArg');
			if($throwException)
