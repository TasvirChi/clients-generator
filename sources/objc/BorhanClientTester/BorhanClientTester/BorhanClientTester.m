...
#import "BorhanMetadataClientPlugin.h"
#import "BorhanClientTester.h"
...
#define BORHAN_CLIENT_TEST_URL (@"@SERVICE_URL@/clientTest")

/*
 BorhanTestDetails
 */
@interface BorhanTestDetails : NSObject
...
@implementation BorhanTestDetails
 ...
 */
@implementation BorhanCallbackDelegate
...
/*
 BorhanDownloadDelegate
 */
@interface BorhanProgressDelegate : NSObject <ASIProgressDelegate> 
...
@implementation BorhanProgressDelegate
...
@implementation BorhanClientTester
...
- (void)initTestsArray
{
    ...
    for (int i = 0; i < methodCount; i++)
    {
        ...
        BorhanTestDetails* details = [[BorhanTestDetails alloc] init];
...
- (id)initWithDelegate:(id <BorhanClientTesterDelegate>)aDelegate;
{
    ...
    self->_client = [[BorhanClient alloc] initWithConfig:config];
    [config release];           // retained on the client
    
    self->_client.ks = [BorhanClient generateSessionWithSecret:ADMIN_SECRET withUserId:USER_ID withType:[BorhanSessionType ADMIN] withPartnerId:PARTNER_ID withExpiry:86400 withPrivileges:@""];
    
    self->_clientDelegate = [[BorhanCallbackDelegate alloc] init];
...
- (BorhanBaseEntry*)uploadEntryWithFileName:(NSString*)fileBase withFileExt:(NSString*)fileExt withMediaType:(int)mediaType
{
    NSString* fileName = [NSString stringWithFormat:@"%@.%@", fileBase, fileExt];
    
    // return: object, params: object
    BorhanUploadToken* token = [[[BorhanUploadToken alloc] init] autorelease];
...
- (void)unexpRequestFailed:(BorhanClientBase *)aClient
{
    assert(NO);
}

- (void)unexpRequestFinished:(BorhanClientBase *)aClient withResult:(id)result
{
    assert(NO);
}
...
- (void)setUp
{
    ...
    self->_imageEntry = [[self uploadEntryWithFileName:@"DemoImage" withFileExt:@"jpg" withMediaType:[BorhanMediaType IMAGE]] retain];

    // -- create a video entry
    self->_videoEntry = [[self uploadEntryWithFileName:@"DemoVideo" withFileExt:@"flv" withMediaType:[BorhanMediaType VIDEO]] retain];
    
    BorhanFlavorAsset* firstFlavor = nil;
    for(;;)
    {
        ...
        if (firstFlavor.status == [BorhanFlavorAssetStatus READY])
            break;
...
- (void)testSyncFlow
{
    ...
    assert(token.status == [BorhanUploadTokenStatus PENDING]);
    ...
    assert([[BorhanEntryStatus NO_CONTENT] compare:entry.status] == NSOrderedSame);
    ...
    entry = [self->_client.media addContentWithEntryId:entry.id withResource:resource];
    assert(self->_client.error == nil);
    assert([[BorhanEntryStatus IMPORT] compare:entry.status] == NSOrderedSame);
    ...
    assert(self->_client.error == nil);
    assert(token.status == [BorhanUploadTokenStatus CLOSED]);
    ...
    assert(flavorArray.count > 0);
    BOOL foundSource = NO;
    for (BorhanFlavorAsset* asset in flavorArray)
    {
    ...
    BorhanMediaEntryFilter* mediaFilter = [[[BorhanMediaEntryFilter alloc] init] autorelease];
    mediaFilter.idEqual = entry.id;
    mediaFilter.statusNotEqual = [BorhanEntryStatus DELETED];
    ...
    BorhanMediaEntryFilterForPlaylist* playlistFilter = [[[BorhanMediaEntryFilterForPlaylist alloc] init] autorelease];
    ...
    BorhanBaseEntry* firstPlaylistEntry = [playlistExecute objectAtIndex:0];
...
- (void)validateSyncMultiReqFlow:(NSArray*)results
{
    ...
    // uploadToken.add
    BorhanUploadToken* token = [results objectAtIndex:1];
    assert(token.id.length > 0);
    assert([token.fileName compare:UPLOAD_FILENAME] == NSOrderedSame);
    assert(token.status == [BorhanUploadTokenStatus PENDING]);
    assert(token.partnerId == PARTNER_ID);
    ...
    // media.add
    BorhanMediaEntry* entry = [results objectAtIndex:2];
    assert(entry.id.length > 0);
    assert([[BorhanEntryStatus NO_CONTENT] compare:entry.status] == NSOrderedSame);
    assert([entry.name compare:ENTRY_NAME] == NSOrderedSame);
    assert(entry.partnerId == PARTNER_ID);
...
- (void)testPremadeRequest
{
    ...
    BorhanClient* client = [[BorhanClient alloc] initWithConfig:config];
    [config release];           // retained on the client
    ...
    // array
    BorhanString* string = [[[BorhanString alloc] init] autorelease];
    string.value = @"dummy";
    NSArray* array = [NSArray arrayWithObject:string];
    [client.params addIfDefinedKey:@"array" withArray:array];
    
    // null / empty items
    [client.params addIfDefinedKey:@"emptyBool" withBool:BORHAN_NULL_BOOL];
    [client.params addIfDefinedKey:@"emptyInt" withInt:BORHAN_NULL_INT];
    [client.params addIfDefinedKey:@"emptyFloat" withFloat:BORHAN_NULL_FLOAT];
    [client.params addIfDefinedKey:@"emptyString" withString:BORHAN_NULL_STRING];
    [client.params addIfDefinedKey:@"emptyObject" withObject:BORHAN_NULL_OBJECT];
    ...
    NSString* expectedResult = [NSString stringWithFormat:@"%@/api_v3?kalsig=b2e9bd151b7edf43c2e210e45ffb15fd&string=strVal&service=test&partnerId=56789&object%%3AobjectType=BorhanMediaEntry&object%%3Aname=abcd&ks=abcdef&int=1234&ignoreNull=1&format=2&emptyString__null=&emptyObject__null=&emptyInt__null=&emptyFloat__null=&emptyBool__null=&emptyArray%%3A-=&clientTag=testTag&bool=0&array%%3A0%%3Avalue=dummy&array%%3A0%%3AobjectType=BorhanString&apiVersion=9.8.7&action=testAct&", DEFAULT_SERVICE_URL];
...
- (void)testHttps
{
    self->_client.config.serviceUrl = @"https://www.borhan.com";
    [self testSyncFlow];
}
...
{
    self->_client.config.serviceUrl = @"http://www.nonexistingborhan.com";
    [self->_client.system ping];
    NSError* error = self->_client.error;
...
- (void)assertBorhanError:(NSError*)error withCode:(int)code
{
    assert(error != nil);
    assert([error.domain compare:BorhanClientErrorDomain] == NSOrderedSame);
    assert(error.code == code);
}

- (void)testSyncApiError
{
    BorhanBaseEntry* entry = [self->_client.baseEntry getWithEntryId:@"NonExistingEntry"];
    assert(entry == nil);
    [self assertBorhanError:self->_client.error withCode:BorhanClientErrorAPIException];
}
...
- (void)testSyncMultiReqApiError
{
    ...
    [self assertBorhanError:res2 withCode:BorhanClientErrorAPIException];
...
{
    self->_client.config.serviceUrl = BORHAN_CLIENT_TEST_URL;
    [self->_client.params addIfDefinedKey:@"responseBuffer" withString:@"<xml>"];
    [self->_client.system ping];
    
    [self assertBorhanError:self->_client.error withCode:BorhanClientErrorXmlParsing];
}
...
- (void)testTagInSimpleType
{
    ...
    [self assertBorhanError:self->_client.error withCode:BorhanClientErrorStartTagInSimpleType];
}
...
- (void)testEmptyObjectOrException
{
    ...
    [self assertBorhanError:self->_client.error withCode:BorhanClientErrorEmptyObject];
}

- (void)testEmptyObject
{
    self->_client.config.serviceUrl = BORHAN_CLIENT_TEST_URL;
    [self->_client.params addIfDefinedKey:@"responseBuffer" withString:@"<xml><result><objectType>BorhanPlaylist</objectType><filters><item/></filters></result></xml>"];
    [self->_client.playlist getWithId:@"1234"];
    
    [self assertBorhanError:self->_client.error withCode:BorhanClientErrorMissingObjectTypeTag];
}

- (void)testTagInSimpleObjectProperty
{
    self->_client.config.serviceUrl = BORHAN_CLIENT_TEST_URL;
    [self->_client.params addIfDefinedKey:@"responseBuffer" withString:@"<xml><result><objectType>BorhanPlaylist</objectType><id><sometag/></id></result></xml>"];
    [self->_client.playlist getWithId:@"1234"];
    
    [self assertBorhanError:self->_client.error withCode:BorhanClientErrorUnexpectedTagInSimpleType];
}
...
- (void)testTagInObjectDoesntStartWithType
{
    ...
    [self assertBorhanError:self->_client.error withCode:BorhanClientErrorExpectedObjectTypeTag];
}
...
- (void)testCharsInsteadOfObject
{
    ...
    [self assertBorhanError:self->_client.error withCode:BorhanClientErrorExpectedPropertyTag];
}
...
- (void)testUnknownObjectType
{
    ...
    [self assertBorhanError:self->_client.error withCode:BorhanClientErrorUnknownObjectType];
}

- (void)testNonBorhanObjectType
{
    self->_client.config.serviceUrl = BORHAN_CLIENT_TEST_URL;
...
- (void)testArrayTagIsNotItem
{
    ...
    [self assertBorhanError:self->_client.error withCode:BorhanClientErrorUnexpectedArrayTag];
}
...
- (void)testMultiReqTagNotItem
{
    ...
    [self assertBorhanError:self->_client.error withCode:BorhanClientErrorUnexpectedMultiReqTag];
}
...
- (void)testMultiReqNotEnoughItems
{
    ...
    [self assertBorhanError:self->_client.error withCode:BorhanClientErrorMissingMultiReqItems];
}

- (void)testInvalidHttpStatus
{
    self->_client.config.serviceUrl = @"http://www.borhan.com/nonExistingFolder";
    [self->_client.system ping];
    
    [self assertBorhanError:self->_client.error withCode:BorhanClientErrorInvalidHttpCode];
}
...
- (void)testDoubleMultiReqStart
{
    ...
    @catch (BorhanClientException *exception) 
    {
...
- (void)testUsePlugin
{
    BorhanMetadataClientPlugin* metadata = [[BorhanMetadataClientPlugin alloc] initWithClient:self->_client];
    BorhanMetadataProfile* profile = [[[BorhanMetadataProfile alloc] init] autorelease];
    ...
    BorhanMetadata* metadataObj = [metadata.metadata addWithMetadataProfileId:profile.id withObjectType:[BorhanMetadataObjectType ENTRY] withObjectId:self->_imageEntry.id withXmlData:METADATA_XML];
    assert(self->_client.error == nil);
...
{
    // int, string
    NSString* ks = [self->_client.session startWithSecret:ADMIN_SECRET withUserId:USER_ID withType:[BorhanSessionType ADMIN] withPartnerId:PARTNER_ID];
    assert(self->_client.error == nil);
    ...
    BorhanMediaEntryFilter* mediaFilter = [[[BorhanMediaEntryFilter alloc] init] autorelease];
    mediaFilter.statusNotEqual = [BorhanEntryStatus DELETED];
    BorhanMediaListResponse* listResult = [self->_client.media listWithFilter:mediaFilter];
    assert(self->_client.error == nil);
...
- (void)callback_testAsyncApiError_RequestFailed:(BorhanClientBase *)aClient
{
    [self assertBorhanError:aClient.error withCode:BorhanClientErrorAPIException];
...
- (void)callback_testAsyncInvalidServerDnsName_RequestFailed:(BorhanClientBase *)aClient
{
...
- (void)callback_testAsyncXmlParsingError_RequestFailed:(BorhanClientBase *)aClient
{
    [self assertBorhanError:self->_client.error withCode:BorhanClientErrorXmlParsing];
...
- (void)callback_testAsyncMultiReqFlow_RequestFinished:(BorhanClientBase *)aClient withResult:(id)aResult
{
    assert(self->_client.error == nil);
...
- (void)callback_testAsyncInvalidHttpStatus_RequestFailed:(BorhanClientBase *)aClient
{
    [self assertBorhanError:self->_client.error withCode:BorhanClientErrorInvalidHttpCode];
...
- (void)callback_testAsyncMultiReqXmlParsingError_RequestFailed:(BorhanClientBase *)aClient
{
    [self assertBorhanError:self->_client.error withCode:BorhanClientErrorXmlParsing];
...
- (void)callback_testAsyncMultiReqApiError_RequestFinished:(BorhanClientBase *)aClient withResult:(id)aResults
{
...
- (void)callback_testAsyncEmptyMultirequest_RequestFinished:(BorhanClientBase *)aClient withResult:(id)aResults
{
    NSArray* results = aResults;
    assert(self->_client.error == nil);
...
- (void)callback_testAsyncBoolType_RequestFinished:(BorhanClientBase *)aClient withResult:(id)aResults
{
...
- (void)callback_testAsyncIntType_RequestFinished:(BorhanClientBase *)aClient withResult:(id)aResults
{
    NSString* result = aResults;
    assert(self->_client.error == nil);
    assert([result compare:@"1"] == NSOrderedSame);    
...
- (void)callback_testAsyncStringType_RequestFinished:(BorhanClientBase *)aClient withResult:(id)aResults
{
    NSString* result = aResults;
    assert(self->_client.error == nil);
...
{
    [self->_client.session startWithSecret:ADMIN_SECRET withUserId:USER_ID withType:[BorhanSessionType ADMIN] withPartnerId:PARTNER_ID];
}

- (void)callback_testAsyncObjectType_RequestFinished:(BorhanClientBase *)aClient withResult:(id)aResults
{
    BorhanBaseEntry* result = aResults;
    assert(self->_client.error == nil);
...
- (void)callback_testAsyncArrayType_RequestFinished:(BorhanClientBase *)aClient withResult:(id)aResults
{
...
- (void)callback_testAsyncVoidType_RequestFinished:(BorhanClientBase *)aClient withResult:(id)aResults
{
    NSString* result = aResults;
    assert(self->_client.error == nil);
...
- (void)testUnknownObjectReturned
{
    ...
    BorhanBaseEntry* result = [self->_client.baseEntry getWithEntryId:self->_imageEntry.id];
    assert(self->_client.error == nil);
    assert([result isKindOfClass:[BorhanBaseEntry class]]);
...
- (void)testUnknownArrayObjectReturned
{
    ...
    BorhanFlavorAsset* asset = [result objectAtIndex:0];
    assert([asset isKindOfClass:[BorhanFlavorAsset class]]);
...
{
    self->_client.config.serviceUrl = BORHAN_CLIENT_TEST_URL;
    [self->_client.params addIfDefinedKey:@"responseBuffer" withString:@"<xml><result><objectType>BorhanConversionProfile</objectType><cropDimensions><objectType>UnknownObjectType</objectType><left>1234</left></cropDimensions></result></xml>"];
    BorhanConversionProfile* result = [self->_client.conversionProfile getWithId:1];
    assert(self->_client.error == nil);
    BorhanCropDimensions* dimensions = result.cropDimensions;
    assert([dimensions isKindOfClass:[BorhanCropDimensions class]]);
...
{
    self->_client.config.serviceUrl = BORHAN_CLIENT_TEST_URL;
    [self->_client.params addIfDefinedKey:@"responseBuffer" withString:@"<xml><result><objectType>BorhanBaseEntryListResponse</objectType><objects><item><objectType>UnknownObjectType</objectType><id>abcdef</id></item></objects></result></xml>"];
    BorhanBaseEntryListResponse* result = [self->_client.baseEntry list];
    assert(self->_client.error == nil);
    assert(result.objects.count == 1);
    BorhanBaseEntry* entry = [result.objects objectAtIndex:0];
    assert([entry isKindOfClass:[BorhanBaseEntry class]]);
    assert([entry.id compare:@"abcdef"] == NSOrderedSame);
}

- (void)testDownloadDelegateSanity
{
    BorhanProgressDelegate* delegate = [[[BorhanProgressDelegate alloc] init] autorelease];
