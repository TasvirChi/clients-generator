...
//        
//        BorhanMediaEntry *mediaEntry = [self.media objectAtIndex:0];
//        NSLog(@"@", mediaEntry);
...
{
	BorhanMediaEntry *mediaEntry1 = (BorhanMediaEntry *)media1;
    BorhanMediaEntry *mediaEntry2 = (BorhanMediaEntry *)media2;
...
- (void)updateMedia:(NSString *)searchStr {
    ...
    for (BorhanMediaEntry *mediaEntry in array) {
    ...
    for (int i = 0; i < [self.media count]; i++) {
        ...
        BorhanMediaEntry *mediaEntry = [self.media objectAtIndex:i];
...
- (IBAction)shareButtonPressed:(UIButton *)button {
    ...
    BorhanMediaEntry *mediaEntry = [self.media objectAtIndex:currentMovieInd];
    ...
    if (button.tag == 0) {
        ...
        if ([MFMailComposeViewController canSendMail]) {
            ...
            [_controller setSubject:@"Borhan"];
            
            NSString *str = [NSString stringWithFormat:@"I just saw this great video on Borhan mobile app, check it out:\n%@", [[Client instance] getShareURL:mediaEntry]];
    ...
    if (tableView.tag == 0) {
        ...
        if (indexPath.row == 0) {
            ...
            if (index < [self.media count]) {
                ...
                BorhanMediaEntry *mediaEntry = [self.media objectAtIndex:index++];
        ...
        if (mostPopular && indexPath.row == 0) {
            ...
            BorhanCategory *_category = [array objectAtIndex:indexPath.row - (mostPopular ? 1 : 0 )];
...
- (void)drawPlayer: (BorhanMediaEntry *)mediaEntry{
