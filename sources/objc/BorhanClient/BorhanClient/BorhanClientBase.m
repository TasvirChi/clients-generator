...
/*
 String constants
 */
static NSString* const BorhanServiceBaseUrl = @"/api_v3";
static NSString* const BorhanServiceFormatXml = @"2";
NSString* const BorhanClientErrorDomain = @"BorhanClientErrorDomain";

/*
 Class BorhanBool
 */
@implementation BorhanBool
 ...
 */
@implementation BorhanClientException
@end

/*
 Class BorhanSimpleTypeParser
 */
@implementation BorhanSimpleTypeParser

+ (BORHAN_BOOL)parseBool:(NSString*)aStr
{
    if (aStr == nil)
        return [BorhanBool NO_VALUE];
    if ([aStr compare:@"1"] != NSOrderedSame)
        return [BorhanBool NO_VALUE];
    return [BorhanBool YES_VALUE];
}
 ...
 */
@implementation BorhanObjectBase
 ...
 */
@implementation BorhanException : BorhanObjectBase
...
- (BorhanFieldType)getTypeOfCode
{
    return KFT_String;
}

- (BorhanFieldType)getTypeOfMessage
{
    return KFT_String;
}
...
- (NSError*)error
{
    return [NSError errorWithDomain:BorhanClientErrorDomain code:BorhanClientErrorAPIException userInfo:[NSDictionary dictionaryWithObjectsAndKeys:self.message, NSLocalizedDescriptionKey, self.code, @"ExceptionCode", nil]];
}
 ...
 */
@implementation BorhanObjectFactory
...
+ (BorhanObjectBase*)createByName:(NSString*)aName withDefaultType:(NSString*)aDefaultType
{
    ...
    if (![objClass isSubclassOfClass:[BorhanObjectBase class]])
    {
        return nil;
    }
...
/*
 Class BorhanParam
 */
@interface BorhanParam : NSObject
...
- (NSComparisonResult)compare:(BorhanParam*)aOtherParam
{
...
{
    for (BorhanParam* param in self->_params)
    {
...
{
    BorhanParam* param = nil;
    
    if (self->_prefix.length != 0)
    {
        aKey = [[NSString alloc] initWithFormat:@"%@%@", self->_prefix, aKey];
    }
    param = [[BorhanParam alloc] initWithKey:aKey withValue:aVal];
    if (self->_prefix.length != 0)
    {
        [aKey release];
    }
...
- (void)addIfDefinedKey:(NSString*)aKey withFileName:(NSString*)aFileName;
{
    ...
    param = [[BorhanFile alloc] initWithKey:aKey withFileName:aFileName];
    if (self->_prefix.length != 0)
    {
        [aKey release];
    }
...
{
    if (aVal == BORHAN_UNDEF_BOOL)
        return;
    
    if (aVal == BORHAN_NULL_BOOL)
    {
...
{
    if (aVal == BORHAN_UNDEF_INT)
        return;
    
    if (aVal == BORHAN_NULL_INT)
    {
        [self putNullKey:aKey];
    }
    else
...
{
    if (isnan(aVal))        // cannot compare to BORHAN_UNDEF_FLOAT since NaN != NaN
        return;

    if (aVal == BORHAN_NULL_FLOAT)
    {
        [self putNullKey:aKey];
    }
    else
...
{
    if (aVal == BORHAN_UNDEF_STRING)
        return;
    
    if ([aVal compare:BORHAN_NULL_STRING] == NSOrderedSame)
    {
        [self putNullKey:aKey];
    }
    else
...
- (void)addIfDefinedKey:(NSString*)aKey withObject:(BorhanObjectBase*)aVal
{
    ...
    if ([aVal isMemberOfClass:[BorhanObjectBase class]])
    {
        [self putNullKey:aKey];
    }
    else
...
- (void)sign
{
    ...
    CC_MD5_Init(&md5Ctx);
    for (BorhanParam* curParam in self->_params)
    {
...
- (void)addToRequest:(ASIFormDataRequest*)aRequest
{
    ...
    {
        BorhanParam* curParam = [self->_params lastObject];
    ...
    {
        BorhanFile* curFile = [self->_files lastObject];
...
- (void)appendQueryString:(NSMutableString*)output
{
    ...
    {
        BorhanParam* curParam = [self->_params lastObject];
        NSString *encodedKey = [BorhanParams allocUrlEncodedString:curParam.key];
        NSString *encodedVal = [BorhanParams allocUrlEncodedString:curParam.value];
 ...
 */
@implementation BorhanServiceBase
 ...
 */
@implementation BorhanClientPlugin
@end

/*
 Class BorhanConfiguration
 */
@implementation BorhanConfiguration
...
- (id)init
{
    ...
    self.partnerId = -1;
    self.serviceUrl = @"http://www.borhan.com";
 ...
 */
@interface BorhanClientBase()

- (id)queueService:(NSString*)aService withAction:(NSString*)aAction withParser:(BorhanXmlParserBase*)aParser;
...
@implementation BorhanClientBase
...
- (id)initWithConfig:(BorhanConfiguration*)aConfig
{
    ...
    self->_params = [[BorhanParams alloc] init];
    self.config = aConfig;
...
- (void)addGlobalParamsAndSign
{
    ...
    [self->_params addIfDefinedKey:@"format" withString:BorhanServiceFormatXml];
...
- (NSString*)queueServeService:(NSString*)aService withAction:(NSString*)aAction
{
    ...
    NSMutableString* result = [[NSMutableString alloc] initWithFormat:@"%@%@?", self.config.serviceUrl, BorhanServiceBaseUrl];
...
- (void)setupRequestWithQuery:(NSString*)aFormat withArguments:(va_list)vaArgs
{
    ...
    NSString* urlStr = [[NSString alloc] initWithFormat:@"%@%@?%@", self.config.serviceUrl, BorhanServiceBaseUrl, query];
...
- (void)setupXmlParser
{
    ...
    self->_skipParser = [[BorhanXmlParserSkipPath alloc] initWithSubParser:self->_reqParser withPath:path];
    [path release];
    
    self->_xmlParser = [[BorhanLibXmlWrapper alloc] init];
...
- (void)queueMultiRequestService:(NSString*)aService withAction:(NSString*)aAction withParser:(BorhanXmlParserBase*)aParser
{
    BorhanXmlParserMultirequest* multiReqParser = (BorhanXmlParserMultirequest*)self->_reqParser;
    [self->_params addIfDefinedKey:@"service" withString:aService];
    [self->_params addIfDefinedKey:@"action" withString:aAction];
...
- (id)queueService:(NSString*)aService withAction:(NSString*)aAction withParser:(BorhanXmlParserBase*)aParser
{
    ...
    BorhanXmlParserException* excParser = [[BorhanXmlParserException alloc] initWithSubParser:aParser];
...
- (void)startMultiRequest
{
    if (self->_isMultiRequest)
    {
        @throw [BorhanClientException exceptionWithName:@"DoubleStartMultiReq" reason:@"startMultiRequest called while already started" userInfo:nil];
    }
    
    self.error = nil;
    self->_reqParser = [[BorhanXmlParserMultirequest alloc] init];
...
- (NSArray*)doMultiRequest
{
    if (!self->_isMultiRequest)
    {
        @throw [BorhanClientException exceptionWithName:@"EndWithoutMultiReq" reason:@"doMultiRequest called while not in multirequest" userInfo:nil];
    }
...
{
    BorhanXmlParserSimpleType* parser = [[BorhanXmlParserSimpleType alloc] init];
...
- (BORHAN_BOOL)queueBoolService:(NSString*)aService withAction:(NSString*)aAction
{
    ...
    [parser release];
    return [BorhanSimpleTypeParser parseBool:result];
}
...
- (int)queueIntService:(NSString*)aService withAction:(NSString*)aAction
{
    ...
    id result = [self queueService:aService withAction:aAction withParser:parser];
    [parser release];
    return [BorhanSimpleTypeParser parseInt:result];
}
...
- (double)queueFloatService:(NSString*)aService withAction:(NSString*)aAction
{
    ...
    id result = [self queueService:aService withAction:aAction withParser:parser];
    [parser release];
    return [BorhanSimpleTypeParser parseFloat:result];
}
...
{
    BorhanXmlParserObject* parser = [[BorhanXmlParserObject alloc] initWithExpectedType:aExpectedType];
    id result = [self queueService:aService withAction:aAction withParser:parser];
    [parser release];
    return result;
}
...
{
    BorhanXmlParserArray* parser = [[BorhanXmlParserArray alloc] initWithExpectedType:aExpectedType];
    id result = [self queueService:aService withAction:aAction withParser:parser];
    [parser release];
    return result;
}
...
- (void)request:(ASIHTTPRequest *)request didReceiveResponseHeaders:(NSDictionary *)responseHeaders
{
    ...
    NSString* sessionName = [responseHeaders objectForKey:@"X-Borhan-Session"];
    ...
    if (statusCode != 200)
    {
        ...
        NSError *nsError = [NSError errorWithDomain:BorhanClientErrorDomain code:BorhanClientErrorInvalidHttpCode userInfo:[NSDictionary dictionaryWithObjectsAndKeys: @"Got invalid HTTP status", NSLocalizedDescriptionKey, statusCodeNum, @"StatusCode", nil]];         
...
+ (NSString*)generateSessionWithSecret:(NSString*)aSecret withUserId:(NSString*)aUserId withType:(int)aType withPartnerId:(int)aPartnerId withExpiry:(int)aExpiry withPrivileges:(NSString*)aPrivileges
{
    ...
    [BorhanClientBase appendSessionSigWithSecret:aSecret withFields:ksFields withOutput:ksWithSig];
