...
import com.borhan.services.FlavorAsset;
...
public class Player extends TemplateActivity implements SurfaceHolder.Callback {
	...
	private com.borhan.player.ViewPlayer viewPlayer;
	...
	private List<BorhanFlavorAsset> listFlavorAssets = new ArrayList<BorhanFlavorAsset>();
	private List<BorhanFlavorAsset> copyListFlavorAssets;
	private String url;
	...
	// viewPlayer = new com.borhan.player.ViewPlayer(TAG, activity, holder,
	// duration, entryId);
	...
	public void surfaceCreated(SurfaceHolder arg0) {		
		...
		try {
			holder.setType(SurfaceHolder.SURFACE_TYPE_PUSH_BUFFERS);
			holder.setFixedSize(400, 300);
			viewPlayer = new com.borhan.player.ViewPlayer(TAG, activity,
	...
	private class DownloadTask extends AsyncTask<Void, States, Void> {
			...
			try {
				if (Utils.checkInternetConnection(getApplicationContext())) {
					...
					try {
						...
						copyListFlavorAssets =  new ArrayList<BorhanFlavorAsset>(); 
						Collections.sort(listFlavorAssets,
								new Sort<BorhanFlavorAsset>("bitrate",
										"reverse"));
						for (BorhanFlavorAsset f : listFlavorAssets) {
						...
						for (BorhanFlavorAsset borhanFlavorAsset : listFlavorAssets) {
							if (!new Integer(
									Math.round(borhanFlavorAsset.bitrate / 100) * 100)
									.equals(0)) {
								copyListFlavorAssets.add(borhanFlavorAsset);
							}
						}
		...
		protected void onPostExecute(Void param) {
			...
			Log.w(TAG, "----------------");
			for (BorhanFlavorAsset f : copyListFlavorAssets) {
