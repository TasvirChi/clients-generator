...
import com.borhan.boxAdapter.BoxAdapterAllEntries;
...
import com.borhan.components.GridForLand;
import com.borhan.components.GridForPort;
import com.borhan.enums.States;
import com.borhan.mediatorActivity.TemplateActivity;
import com.borhan.services.Media;
import com.borhan.utils.SearchTextEntry;
...
public class VideoCategory extends TemplateActivity implements Observer {
    ...
    private List<BorhanMediaEntry> copyEntries;
    ...
    private BorhanMediaEntry lastCreatedEntry;
    ...
    private BorhanMediaEntry rightTopEntry;
    ...
    private class DownloadListCatigoriesTask extends AsyncTask<Void, States, List<BorhanMediaEntry>> {

        private String message;

        @Override
        protected List<BorhanMediaEntry> doInBackground(Void... params) {
            // Test for connection
            try {
        ...
        @Override
        protected void onPostExecute(List<BorhanMediaEntry> listCategory) {
            progressDialog.hide();
            //setRequestedOrientation(ActivityInfo.SCREEN_ORIENTATION_UNSPECIFIED);
    ...
    private void ImageLoader(BorhanMediaEntry lastCreatedEntry) {
        ...
        for (BorhanMediaEntry entry : copyEntries) {
    ...
    private void updateData(List<BorhanMediaEntry> listEntries) {
        copyEntries = new ArrayList<BorhanMediaEntry>();
        ...
        if (copyEntries.size() > 0) {
            ...
            Collections.sort(copyEntries, new Sort<BorhanMediaEntry>("createdAt", "compareTo"));
            lastCreatedEntry = copyEntries.get(copyEntries.size() - 1);
            copyEntries.remove(lastCreatedEntry);
            Collections.sort(copyEntries, new Sort<BorhanMediaEntry>("name", "compareTo"));
            addContentLastEntry();
        } else {
    ...
    public void update(Observable paramObservable, Object paramObject) {
        updateData((List<BorhanMediaEntry>) paramObject);
    }
}
