package com.borhan.player;

import java.util.ArrayList;
import com.borhan.client.types.BorhanWidevineFlavorAsset;
...
import com.borhan.boxAdapter.BoxAdapterRate;
import com.borhan.client.types.BorhanFlavorAsset;
import com.borhan.services.AdminUser;
import com.borhan.utils.Utils;
import com.borhan.widevine.WidevineHandler;
...
public class ViewPlayer implements Observer, OnClickListener, SeekBar.OnSeekBarChangeListener, AdapterView.OnItemClickListener {
     ...
     */
    public void addListRates(List<BorhanFlavorAsset> listRates) {
        if (listRates != null) {
            boxAdapterRate = new BoxAdapterRate(activity, listRates, R.drawable.background_selected_rate);
        } else {
            listRates = new ArrayList<BorhanFlavorAsset>();
            boxAdapterRate = new BoxAdapterRate(activity, listRates, R.drawable.background_selected_rate);
        }
        boxAdapterRate.setVisiblityHighlight(true);
        boxAdapterRate.notifyDataSetChanged();
    ...
    public void selectBitrate(int position, int state) {
    	BorhanFlavorAsset flavor = boxAdapterRate.getFlavor(position);
        flavorId = flavor.id;
        Log.w(TAG, flavorId);
        ...
        if (flavor instanceof BorhanWidevineFlavorAsset) {
