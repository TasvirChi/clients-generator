...
BORHAN_SERVICE_FORMAT_JSON = 1
BORHAN_SERVICE_FORMAT_XML  = 2
BORHAN_SERVICE_FORMAT_PHP  = 3
...
# Borhan objects factory
class BorhanObjectFactory(object):
    objectFactories = {}
    ...
    def create(objectNode, expectedType):
        ...
        if not BorhanObjectFactory.objectFactories.has_key(objType):
            objType = expectedType.__name__        
        result = BorhanObjectFactory.objectFactories[objType]()
        if not isinstance(result, expectedType):
            raise BorhanClientException("Unexpected object type '%s'" % objType, BorhanClientException.ERROR_INVALID_OBJECT_TYPE)
    ...
    def createArray(arrayNode, expectedElemType):
        ...
        for arrayElemNode in arrayNode.childNodes:
            results.append(BorhanObjectFactory.create(arrayElemNode, expectedElemType))
        return results
    ...
    def createMap(mapNode, expectedElemType):
        ...
        for mapElemNode in mapNode.childNodes:
            ...
            results[key] = BorhanObjectFactory.create(mapElemNode, expectedElemType)
        return results

    @staticmethod
    def registerObjects(objs):
        BorhanObjectFactory.objectFactories.update(objs)

# Abstract base class for all client objects
class BorhanObjectBase(object):
    def __init__(self, 
            relatedObjects=NotImplemented):

        # @var map of BorhanListResponse
        # @readonly
        self.relatedObjects = relatedObjects
        
        from BorhanClient.Plugins.Core import BorhanListResponse
        BorhanObjectBase.PROPERTY_LOADERS = {
            'relatedObjects': (BorhanObjectFactory.createMap, BorhanListResponse) 
        }
    ...
    def fromXml(self, node):
        self.fromXmlImpl(node, BorhanObjectBase.PROPERTY_LOADERS)
        pass
    
    def toParams(self):
        result = BorhanParams()
        result.put('objectType', 'BorhanObjectBase')
        return result
...
class BorhanServiceBase(object):
...
class BorhanException(Exception):
...
class BorhanClientException(Exception):
    ERROR_GENERIC = -1
    ERROR_INVALID_XML = -2
...
class BorhanConfiguration(object):
    # Constructs new Borhan configuration object
    def __init__(self, serviceUrl = "http://www.borhan.com", logger = None):
        self.logger                     = logger
        self.serviceUrl                 = serviceUrl
        self.format                     = BORHAN_SERVICE_FORMAT_XML
...
class IBorhanClientPlugin(object):
...
class BorhanClientPlugin(IBorhanClientPlugin):
    pass

# Borhan enums factory
class BorhanEnumsFactory(object):
    enumFactories = {}

    @staticmethod
    def create(enumValue, enumType):
        if not BorhanEnumsFactory.enumFactories.has_key(enumType):
            raise BorhanClientException("Unrecognized enum '%s'" % enumType, BorhanClientException.ERROR_INVALID_OBJECT_TYPE)
        return BorhanEnumsFactory.enumFactories[enumType](enumValue)
    ...
    def createInt(enumNode, enumType):
        ...
        if enumValue == None:
            return None
        return BorhanEnumsFactory.create(enumValue, enumType)
    ...
    def registerEnums(objs):
        BorhanEnumsFactory.enumFactories.update(objs)

# Implement to get Borhan Client logs
class IBorhanLogger(object):
    def log(self, msg):
        raise NotImplementedError
