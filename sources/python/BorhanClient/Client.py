...
from BorhanClient.Plugins.Core import BorhanClientConfiguration
from BorhanClient.Plugins.Core import BorhanRequestConfiguration
...
class BorhanClient(object):
    ...
    def loadConfigurations(self):
        self.loadConfiguration(BorhanClientConfiguration, self.clientConfiguration)
        self.loadConfiguration(BorhanRequestConfiguration, self.requestConfiguration)
    ...
    def loadPlugin(self, pluginFile):
        moduleHierarchy = ['BorhanClient', 'Plugins', pluginFile]
        ...
        if pluginFile == 'Core':
            pluginClass = 'BorhanCoreClient'
        else:
            pluginClass = 'Borhan%sClientPlugin' % pluginFile
        ...
        if not isinstance(plugin, IBorhanClientPlugin):
            return
    ...
    def registerPluginObjects(self, plugin):
        BorhanEnumsFactory.registerEnums(plugin.getEnums())
        BorhanObjectFactory.registerObjects(plugin.getTypes())
    ...
    def queueServiceActionCall(self, service, action, returnType, params = BorhanParams(), files = BorhanFiles()):
        for param in self.requestConfiguration:
            if isinstance(self.requestConfiguration[param], BorhanObjectBase):
                params.addObjectIfDefined(param, self.requestConfiguration[param])
            else:
                params.put(param, self.requestConfiguration[param])
                
        call = BorhanServiceActionCall(service, action, params, files)
    ...
    def openRequestUrl(url, params, files, requestHeaders):
        ...
        try:
            ...
            raise BorhanClientException(e, BorhanClientException.ERROR_CONNECTION_FAILED)
        return f

    @staticmethod
    def readHttpResponse(f, requestTimeout):
        if requestTimeout != None:
            readTimer = Timer(requestTimeout, BorhanClient.closeHandle, [f])
            readTimer.start()
        try:
            try:
                data = f.read()
            except AttributeError, e:      # socket was closed while reading
                raise BorhanClientException(e, BorhanClientException.ERROR_READ_TIMEOUT)
            except Exception, e:
                raise BorhanClientException(e, BorhanClientException.ERROR_READ_FAILED)
            if f.info().get('Content-Encoding') == 'gzip':
                gzipFile = gzip.GzipFile(fileobj=StringIO(data))
                try:
                    data = gzipFile.read()
                except IOError, e:
                    raise BorhanClientException(e, BorhanClientException.ERROR_READ_GZIP_FAILED)
        finally:
            if requestTimeout != None:
                readTimer.cancel()
        return data
    ...
    # Send http request
    def doHttpRequest(self, url, params = BorhanParams(), files = BorhanFiles()):
        if len(files.get()) == 0:
    ...
    def parsePostResult(self, postResult):
        ...
        try:        
            ...
            raise BorhanClientException(e, BorhanClientException.ERROR_INVALID_XML)
            
        resultNode = getChildNodeByXPath(resultXml, 'xml/result')
        if resultNode == None:
            raise BorhanClientException('Could not find result node in response xml', BorhanClientException.ERROR_RESULT_NOT_FOUND)
    ...
    def doQueue(self):
        ...
        if self.config.format != BORHAN_SERVICE_FORMAT_XML:
            raise BorhanClientException("unsupported format: %s" % (postResult), BorhanClientException.ERROR_FORMAT_NOT_SUPPORTED)
        ...
        for curHeader in self.responseHeaders:
            ...
            elif curHeader.startswith('X-Borhan-Session:'):
    ...
    def setConfig(self, config):
        self.config = config
        logger = self.config.getLogger()
        if isinstance(logger, IBorhanLogger):
            self.shouldLog = True
    ...
    def getExceptionIfError(self, resultNode):
        ...
        if messageNode == None or codeNode == None:
            return None
        return BorhanException(getXmlNodeText(messageNode), getXmlNodeText(codeNode))
    ...
    def doMultiRequest(self):
        ...
        for childNode in resultXml.childNodes:
            ...
            if exceptionObj != None:
                ...
                result.append(BorhanObjectFactory.create(childNode, self.multiRequestReturnType[i]))
            elif getChildNodeByXPath(childNode, 'item/objectType') != None:
                result.append(BorhanObjectFactory.createArray(childNode, self.multiRequestReturnType[i]))
            else:
    ...
    def generateSession(adminSecretForSigning, userId, type, partnerId, expiry = 86400, privileges = ''):
        ...
        info = ';'.join(fields)
        signature = BorhanClient.hash(adminSecretForSigning + info).encode('hex')
    ...
    def generateSessionV2(adminSecretForSigning, userId, type, partnerId, expiry = 86400, privileges = ''):
        ...
        fields[BorhanClient.FIELD_EXPIRY] = str(int(time.time()) + expiry)
        fields[BorhanClient.FIELD_TYPE] = str(type)
        fields[BorhanClient.FIELD_USER] = str(userId)

        # build fields string
        fieldsStr = urllib.urlencode(fields)
        fieldsStr = Random.get_random_bytes(BorhanClient.RANDOM_SIZE) + fieldsStr
        fieldsStr = BorhanClient.hash(fieldsStr) + fieldsStr

        # encrypt and encode
        cipher = AES.new(BorhanClient.hash(adminSecretForSigning)[:16], AES.MODE_CBC, '\0' * 16)
...
class BorhanServiceActionCall(object):
    def __init__(self, service, action, params = BorhanParams(), files = BorhanFiles()):
    ...
    def getParamsForMultiRequest(self, multiRequestIndex):
        ...
        multiRequestParams = BorhanParams()
    ...
    def getFilesForMultiRequest(self, multiRequestIndex):
        multiRequestParams = BorhanFiles()
