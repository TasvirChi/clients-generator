...
define("BORHAN_SERVICE_FORMAT_JSON", 1);
define("BORHAN_SERVICE_FORMAT_XML",  2);
define("BORHAN_SERVICE_FORMAT_PHP",  3);
...
class BorhanClientBase 
{
	 ...
	 */
	function BorhanClientBase(/*BorhanConfiguration*/ $config)
	{
		$this->config = $config;
	...
	function callService($service, $action, $params)
	{
		...
		else 
		{
			...
			if ($this->config->format == BORHAN_SERVICE_FORMAT_PHP)
			{
...
class BorhanServiceBase
{
	 ...
	 */
	function BorhanServiceBase(/*BorhanClient*/ &$client)
	{
		$this->client = &$client;
	}
}
...
class BorhanConfiguration
{
	...
	var $serviceUrl    = "http://www.borhan.com/";
	var $partnerId     = null;
	var $format        = 3;
