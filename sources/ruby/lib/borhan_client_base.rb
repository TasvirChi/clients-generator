...
module Borhan
	class BorhanNotImplemented; end
	...
	class BorhanClientBase
		...
		def queue_service_action_call(service, action, return_type, params = {}, files = {})
			...
			call = BorhanServiceActionCall.new(service, action, return_type, params, files);
		...
		def do_queue()
			begin
				...
				log("server: [" + result.headers[:x_me].to_s + "], session: [" + result.headers[:x_borhan_session].to_s + "]")
			...
			rescue BorhanAPIError => e
				raise e
			rescue Exception => e
				raise BorhanAPIError.new("BORHAN_RUBY_CLIENT_ERROR", e.to_s)
			end
		end
				...
				if xml_element.elements[1].name == 'item' # array or map
					if (xml_element.elements[1].elements['itemKey'].nil?) # array
						...
						xml_element.elements.each('item') do | element |
							instance.push(BorhanClientBase.object_from_xml(element, return_type))
						end
					else # map
						instance = {}
						xml_element.elements.each('item') do | element |
							item_key = element.get_text('itemKey').to_s
							instance[item_key] = BorhanClientBase.object_from_xml(element, return_type)
						end
					end
				else # object
					...
					if (object_type_element != nil)
						...
						borhanModule = Module.const_get("Borhan")
						
						begin
							instance = borhanModule.const_get(object_class).new
						rescue NameError => e
							if(return_type != nil)
								instance = borhanModule.const_get(return_type).new
							else
								raise e
							end
						end
						...
						if (error_element != nil)
							...
							instance = BorhanAPIError.new(code, message)
						end
					end
				end
		...
		def parse_xml_to_objects(xml)
			...
			if (@is_multirequest)
				...
				doc.elements.each('xml/result/*') do | element |
					results[request_index] = BorhanClientBase.object_from_xml(element, @calls_queue[request_index].return_type)
					request_index += 1
				end
				return results
			else
				doc.elements.each('xml/result') do | element |
					return BorhanClientBase.object_from_xml(element, @calls_queue.first.return_type)
				end
			end
		end
		...
		def raise_exception_if_error(doc)
			if is_error(doc)
				...
				raise BorhanAPIError.new(code, message)
			end
		end
			...
			if value == BorhanNotImplemented
				...
				if value.empty?
					...
					value.each_with_index do |ele, i|
						if ele.is_a? BorhanObjectBase
							add_param(params[name], i, ele.to_params)
						end
					end
				end
			elsif value.is_a? BorhanObjectBase
		...
		def generate_session(admin_secret, user_id, borhan_session_type, partner_id, expiry=86400, privileges=nil)

			session = "#{partner_id};#{partner_id};#{Time.now.to_i + expiry};#{borhan_session_type};#{rand.to_s.gsub("0.", "")};#{user_id};#{privileges};"
	...
	class BorhanObjectBase
		...
		def from_xml(xml_element)
			self.related_objects = BorhanClientBase.object_from_xml(xml_element.elements['relatedObjects'], 'BorhanListResponse')
		end
		...
		def to_params
			...
			instance_variables.each do |var|
				...
				kvar = camelcase(var)
				if (value != nil)
					if (value.is_a? BorhanObjectBase)
	...
	class BorhanConfiguration
		...
		#
		# Adding service_url to the initialize signature to pass url to your own borhan ce instance
		# Default is still set to http://www.borhan.com.
		#
		def initialize()
			@service_url 	= "http://www.borhan.com"
			@format 		= 2 # xml
			@timeout 		= 120
			@requestHeaders = {}
		end
	...
	class BorhanAPIError < RuntimeError
		attr_reader :code
		attr_reader :message
