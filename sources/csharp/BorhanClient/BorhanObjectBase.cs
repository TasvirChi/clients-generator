...
namespace Borhan
{
    public class BorhanObjectBase : INotifyPropertyChanged
    {
        #region Private Fields
        private IDictionary<string, BorhanListResponse> _RelatedObjects;
        #endregion

        #region Properties
        public IDictionary<string, BorhanListResponse> RelatedObjects
        {
        ...
        public BorhanObjectBase(XmlElement node)
        {
            foreach (XmlElement propertyNode in node.ChildNodes)
            {
                ...
                switch (propertyNode.Name)
                {
                    case "relatedObjects":
                        {
                            string key;
                            this.RelatedObjects = new Dictionary<string, BorhanListResponse>();
                            foreach (XmlElement arrayNode in propertyNode.ChildNodes)
                            {
                                key = arrayNode["itemKey"].InnerText;
                                this.RelatedObjects[key] = (BorhanListResponse)BorhanObjectFactory.Create(arrayNode, "BorhanListResponse");
                            }
                        }
                        continue;
                }
        ...
        #region Methods
        public virtual BorhanParams ToParams()
        {
