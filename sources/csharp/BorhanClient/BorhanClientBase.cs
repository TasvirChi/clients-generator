...
namespace Borhan
{
    public class BorhanClientBase
    {
        ...
        protected BorhanConfiguration _Config;
        private bool _ShouldLog;
        private List<BorhanServiceActionCall> _CallsQueue;
        private List<string> _MultiRequestReturnType;
        private BorhanParams _MultiRequestParamsMap;
        ...
        public BorhanClientBase(BorhanConfiguration config)
        {
            ...
            _CallsQueue = new List<BorhanServiceActionCall>();
            _MultiRequestParamsMap = new BorhanParams();
        }
        ...
        public void QueueServiceCall(string service, string action, string fallbackClass, BorhanParams kparams)
        {
            this.QueueServiceCall(service, action, fallbackClass, kparams, new BorhanFiles());
        }

        public void QueueServiceCall(string service, string action, string fallbackClass, BorhanParams kparams, BorhanFiles kfiles)
        {
            Object value;
            foreach (string param in requestConfiguration.Keys)
            {
                value = requestConfiguration[param];
                if (value is BorhanObjectBase)
                {
                    kparams.Add(param, ((BorhanObjectBase)value).ToParams());
                }
                else
                {
        ...
        public XmlElement DoQueue()
        {
            ...
            if (_MultiRequestReturnType != null)
            {
                ...
                int i = 1;
                foreach (BorhanServiceActionCall call in _CallsQueue)
                {
                ...
                // map params
                foreach (KeyValuePair<string, IBorhanSerializable> item in _MultiRequestParamsMap)
                {
                    string requestParam = item.Key;
                    IBorhanSerializable resultParam = item.Value;
            ...
            {
                BorhanServiceActionCall call = _CallsQueue[0];
            ...
            using (WebResponse response = request.GetResponse())
            {
                ...
                for (int i = 0; i < this._ResponseHeaders.Count; ++i)
                {
                    ...
                    if (this._ResponseHeaders.Keys[i] == "X-Borhan-Session")
        ...
        private void createProxy(HttpWebRequest request, BorhanConfiguration _Config)
        {
        ...
        public BorhanMultiResponse DoMultiRequest()
        {
        ...
        private BorhanMultiResponse ParseMultiRequestResult(XmlElement childNode) 
        {
            return ParseMultiRequestResult(childNode, true);
        }
        
        private BorhanMultiResponse ParseMultiRequestResult(XmlElement childNode, bool incrementI)  
        {
            int i = 0;
            BorhanMultiResponse multiResponse = new BorhanMultiResponse();
            foreach(XmlElement arrayNode in childNode.ChildNodes) 
            {
                ...
                if (error != null && error["code"] != null && error["message"] != null) 
                    multiResponse.Add(new BorhanAPIException(error["code"].InnerText, error["message"].InnerText));
                else if (arrayNode["objectType"] != null)
                    multiResponse.Add(BorhanObjectFactory.Create(arrayNode, _MultiRequestReturnType[i]));
        ...
        {
            return this.GenerateSession(adminSecretForSigning, userId, (BorhanSessionType)(0));
        }
        ...
        public string GenerateSession(string adminSecretForSigning, string userId, BorhanSessionType type, int partnerId)
        {
            return this.GenerateSession(adminSecretForSigning, userId, type, partnerId, 86400);
        }

        public string GenerateSession(string adminSecretForSigning, string userId, BorhanSessionType type, int partnerId, int expiry)
        {
            return this.GenerateSession(adminSecretForSigning, userId, type, partnerId, expiry, "");
        }

        public string GenerateSession(string adminSecretForSigning, string userId, BorhanSessionType type, int partnerId, int expiry, string privileges)
        {
        ...
        private string Signature(BorhanParams kparams)
        {
        ...
        private void ThrowExceptionOnAPIError(XmlElement result)
        {
            ...
            {
                resetRequest();
                throw new BorhanAPIException(error["code"].InnerText, error["message"].InnerText);
            }
        }

        private void PostMultiPartWithFiles(HttpWebRequest request, string json, BorhanFiles kfiles)
        {
