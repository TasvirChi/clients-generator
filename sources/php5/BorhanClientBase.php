	...
	*/
	...
	public function doQueue()
	{
		...
		else
		{
			...
			elseif ($this->config->format == self::BORHAN_SERVICE_FORMAT_JSON)
			{
	...
	public function setClientConfiguration(BorhanClientConfiguration $configuration)
	{
		$params = get_class_vars('BorhanClientConfiguration');
	...
	public function setRequestConfiguration(BorhanRequestConfiguration $configuration)
	{
		$params = get_class_vars('BorhanRequestConfiguration');
		foreach($params as $param)
		{
			if(is_null($configuration->$param))
			{
	...
	public function jsObjectToClientObject($value)
	{
		...
		if(is_object($value))
		{
			if(isset($value->message) && isset($value->code))
			{
				...
				throw new BorhanException($value->message, $value->code, $value->args);
			}
			
			if(!isset($value->objectType))
			{
				throw new BorhanClientException("Response format not supported - objectType is required for all objects", BorhanClientException::ERROR_FORMAT_NOT_SUPPORTED);
			}
	...
	public function throwExceptionIfError($resultObject)
	{
		...
		{
			throw new BorhanException($resultObject["message"], $resultObject["code"], $resultObject["args"]);
		}
	}
	...
	public function validateObjectType($resultObject, $objectType)
	{
		...
		else if ( is_object($resultObject) )
		{
			...
			{
				throw new BorhanClientException("Invalid object type - not instance of $objectType", BorhanClientException::ERROR_INVALID_OBJECT_TYPE);
			}
		}
		else if(class_exists($objectType) && is_subclass_of($objectType, 'BorhanEnumBase'))
		{
			$enum = new ReflectionClass($objectType);
			$values = array_map('strval', $enum->getConstants());
			if(!in_array($resultObject, $values))
			{
				throw new BorhanClientException("Invalid enum value", BorhanClientException::ERROR_INVALID_ENUM_VALUE);
			}
		}
...
class BorhanConfiguration
{
	...
	public $format        				= BorhanClientBase::BORHAN_SERVICE_FORMAT_PHP;
