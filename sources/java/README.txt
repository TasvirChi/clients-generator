...
== CONTENTS OF THIS PACKAGE ==
 ...
 - Borhan Java Client Library API (src/main/java/com)
...
After running the command you will find 3 Jar files in the "target" directory.
  -- target/borhanClient-X.X.X.jar contains the compiled client library
  -- target/borhanClient-X.X.X-sources.jar contains the source code
  -- target/borhanClient-X.X.X-javadoc.jar contains the Javadoc documentation for the library
...
target/surefire-reports/com.borhan.client.test.BorhanTestSuite.txt
...
To run the main class (Borhan.java):
 - Edit the src/com/borhan/client/test/test.properties file, enter valid data to ENDPOINT, PARTNER_ID, SECRET and ADMIN_SECRET variables.
...
There is a log4j.properties file in src/test/resources/log4j. 
 - Edit it to set the log level as desired, defaults are:
  log4j.category.BorhanClientBase.class=DEBUG
  log4j.logger.com.borhan=ERROR
