...
public final class ParseUtils {
	...
	public static <T> HashMap<String, T> parseMap(Class<T> clz, Node aNode) throws BorhanApiException{
		...
		for (int j = 0; j < subNodeList.getLength(); j++) {
			...
			if(itemNode instanceof Element){
				...
				tmpMap.put(name, (T) BorhanObjectFactory.create((Element) itemNode, clz));
			}
		}
		return tmpMap;
	}
