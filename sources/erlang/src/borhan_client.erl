...
add_params(Params, Key, BorhanObject) when is_tuple(BorhanObject) ->
	PropList = borhan_object_to_proplist(BorhanObject),
	Value = add_params([], PropList),
	add_params(Params, [{Key, Value}]);
add_params(Params, Key, Value) ->
	add_params(Params, [{Key, Value}]).

-spec request(ClientConfiguration::#borhan_configuration{}, ClientRequest::#borhan_request{}, Params::body()) -> Response::response().
request(ClientConfiguration, ClientRequest, Params) ->
	Params1 = add_params(Params, borhan_request_to_proplist(ClientRequest)),
...
request(ClientConfiguration, Body) ->
	Url = ClientConfiguration#borhan_configuration.url,
    Headers = [{"Accept", "application/json"}, {"Content-Type", "application/json"}],
    Request = get_request(Url, Headers,  Body),
    httpc:set_options(ClientConfiguration#borhan_configuration.client_options),
    Response = parse_response(httpc:request(post, Request, ClientConfiguration#borhan_configuration.request_options, [{body_format, binary}])),
	case Response of
		{ok, _, _, Results} ->
			Results;
...
parse_response({ok, {_, Headers, Body}}) ->
	SessionId = proplists:get_value("x-borhan-session", Headers),
