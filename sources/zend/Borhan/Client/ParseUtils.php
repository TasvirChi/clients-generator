...
class Borhan_Client_ParseUtils 
{
	...
	public static function unmarshalObject(\SimpleXMLElement $xml, $fallbackType = null) 
	{
		...
		$type = Borhan_Client_TypeMap::getZendType($objectType);
		if(!class_exists($type)) {
			$type = Borhan_Client_TypeMap::getZendType($fallbackType);
			if(!class_exists($type))
				throw new Borhan_Client_ClientException("Invalid object type class [$type] of Borhan type [$objectType]", Borhan_Client_ClientException::ERROR_INVALID_OBJECT_TYPE);
		}
