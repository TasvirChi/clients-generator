 ...
 * @subpackage Client
 */
class Borhan_Client_ClientBase
{
	...
	/**
	 * @var Borhan_Client_Configuration
	 */
	protected $config;
	...
	/**
	 * @var array<Borhan_Client_ServiceActionCall>
	 */
	 ...
	 *
	 * @param Borhan_Client_Configuration $config
	 */
	public function __construct(Borhan_Client_Configuration $config)
	{
		$this->config = $config;
	...
	public function queueServiceActionCall($service, $action, $returnType, $params = array(), $files = array())
	{
		...
		$call = new Borhan_Client_ServiceActionCall($service, $action, $params, $files);
	...
	public function doQueue()
	{
		...
		if ($error || ($errorCode != 200 ))
		{
			$error .= ". RC : $errorCode";
			$this->resetRequest();
			throw $this->getBorhanClientException($error, Borhan_Client_ClientException::ERROR_GENERIC);
		}
		else
		{
			...
			if ($this->config->format != self::BORHAN_SERVICE_FORMAT_XML)
			{
				$this->resetRequest();
				throw $this->getBorhanClientException("unsupported format: $postResult", Borhan_Client_ClientException::ERROR_FORMAT_NOT_SUPPORTED);
			}
		}
	...
	private function doCurl($url, $params = array(), $files = array())
	{
		...
		if($this->config->format == self::BORHAN_SERVICE_FORMAT_JSON)
		{
	...
	{
		if (count($files) > 0)
			throw $this->getBorhanClientException("Uploading files is not supported with stream context http request, please use curl", Borhan_Client_ClientException::ERROR_UPLOAD_NOT_SUPPORTED);
		...
		if (isset($this->config->proxyType) && $this->config->proxyType === 'SOCKS5') {
			throw $this->getBorhanClientException("Cannot use SOCKS5 without curl installed.", Borhan_Client_ClientException::ERROR_CONNECTION_FAILED);
		}
		if (isset($this->config->proxyHost)) {
		...
		if (!$fp) {
			$phpErrorMsg = "";
			throw $this->getBorhanClientException("Problem with $url, $phpErrorMsg", Borhan_Client_ClientException::ERROR_CONNECTION_FAILED);
		}
		$response = @stream_get_contents($fp);
		if ($response === false) {
			throw $this->getBorhanClientException("Problem reading data from $url, $phpErrorMsg", Borhan_Client_ClientException::ERROR_READ_FAILED);
		}
		return array($response, 200, '');
	}

	/**
	 * @return Borhan_Client_Configuration
	 */
	 ...
	 * @param Borhan_Client_Configuration $config
	 */
	public function setConfig(Borhan_Client_Configuration $config)
	{
		$this->config = $config;

		$logger = $this->config->getLogger();
		if ($logger instanceof Borhan_Client_ILogger)
		{
			$this->shouldLog = true;
		}
	}

	public function setClientConfiguration(Borhan_Client_Type_ClientConfiguration $configuration)
	{
		$params = get_class_vars('Borhan_Client_Type_ClientConfiguration');
	...
	public function setRequestConfiguration(Borhan_Client_Type_RequestConfiguration $configuration)
	{
		$params = get_class_vars('Borhan_Client_Type_RequestConfiguration');
		foreach($params as $param)
		{
			if(is_null($configuration->$param))
			{
	...
	public function addParam(array &$params, $paramName, $paramValue)
	{
		...
		if(is_object($paramValue) && $paramValue instanceof Borhan_Client_ObjectBase)
		{
			$params[$paramName] = array(
				'objectType' => $paramValue->getBorhanObjectType()
			);
	...
	public function jsObjectToClientObject($value)
	{
		...
		if(is_object($value))
		{
			if(isset($value->message) && isset($value->code))
			{
				...
				throw $this->getBorhanAPIException($value->message, $value->code, $value->args);
			}

			if(!isset($value->objectType))
			{
				throw $this->getBorhanClientException("Response format not supported - objectType is required for all objects", Borhan_Client_ClientException::ERROR_FORMAT_NOT_SUPPORTED);
			}
	...
	protected function unsetNull($object)
	{
		...
		if(is_object($object) && $object instanceof Borhan_Client_ObjectBase)
			$array['objectType'] = $object->getBorhanObjectType();
	...
	public function throwExceptionIfError($resultObject)
	{
		...
		{
			throw $this->getBorhanAPIException($resultObject["message"], $resultObject["code"], $resultObject["args"]);
		}
	}
	...
	public function validateObjectType($resultObject, $objectType)
	{
		...
		else if ( is_object($resultObject) )
		{
			...
			{
				throw $this->getBorhanClientException("Invalid object type - not instance of $objectType", Borhan_Client_ClientException::ERROR_INVALID_OBJECT_TYPE);
			}
		}
		else if(class_exists($objectType) && is_subclass_of($objectType, 'Borhan_Client_EnumBase'))
		{
			$enum = new ReflectionClass($objectType);
			$values = array_map('strval', $enum->getConstants());
			if(!in_array($resultObject, $values))
			{
				throw $this->getBorhanClientException("Invalid enum value", Borhan_Client_ClientException::ERROR_INVALID_ENUM_VALUE);
			}
		}
		else if(gettype($resultObject) !== $objectType)
		{
			throw $this->getBorhanClientException("Invalid object type", Borhan_Client_ClientException::ERROR_INVALID_OBJECT_TYPE);
		}
	}
	...
	public function doMultiRequest()
	{
		...
		foreach($items as $item) {
			...
			if($error)
				...
				$ret[] = Borhan_Client_ParseUtils::unmarshalObject($item, $this->multiRequestReturnType[$i]);
			else if($item->item)
				$ret[] = Borhan_Client_ParseUtils::unmarshalArray($item, $this->multiRequestReturnType[$i]);
			else
				$ret[] = Borhan_Client_ParseUtils::unmarshalSimpleType($item);
			$i++;
		}
	...
	{
		return new Borhan_Client_MultiRequestSubResult($this->getMultiRequestQueueSize() . ':result');
	}
	...
	public function getBorhanAPIException($errorMsg, $errorCode, $args)
	{
		return new Borhan_Client_Exception($errorMsg, $errorCode, $args);
	}

	public function getBorhanClientException( $error, $errorCode)
	{
		return new Borhan_Client_ClientException($error, $errorCode);
	}
	...
	public function checkIfError(\SimpleXMLElement $xml, $throwException = true)
	{
		if(($xml->error) && (count($xml->children()) == 1))
		{
			...
			$arguments = Borhan_Client_ParseUtils::unmarshalArray($xml->error->args, 'BorhanApiExceptionArg');
			if($throwException)
				throw $this->getBorhanAPIException($message, $code, $arguments);
			else
				return $this->getBorhanAPIException($message, $code, $arguments);
		}
	}
}
