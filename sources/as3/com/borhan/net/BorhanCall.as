	...
	public class BorhanCall extends EventDispatcher {
		...
		public function setRequestArgument(name:String, value:*):void {
			if (value is Number)
			{
				...
				if (value == BorhanClient.NULL_NUMBER ) { this.args[name + '__null'] = '';  return; }
			}
			if ( value is int)
			{
				if (value == int.MIN_VALUE ) { return; }
				if (value == BorhanClient.NULL_INT ) { this.args[name + '__null'] = '';  return; }
			}
			if (value === null) {	return;	 }
			if (value === BorhanClient.NULL_STRING) {	this.args[name + '__null'] = '';  return;	 }
		...
		protected function borhanObject2Arrays( obj : Object , prefix : String = null) : Array {	
			...
			for (var i:int=0; i<objKeys.length; i++)
			{
				...
				else //must be a Borhan Object
				{
					objArr= getQualifiedClassName(value).split("::");
					var tempPrefix : String = objKeys[i].toString();
					var tempKeyValArr : Array = borhanObject2Arrays( value , prefix + ":" + tempPrefix );
