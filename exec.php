<?php 
/**
 * Client Libraries Generator - generate all the clients specified in the given config.ini according to Borhan API reflection.
 * 
 ...
 * 
 * Notes:
 * 		* Borhan API ignores only un-sent parameters. Thus, if you would like a parameter value to be left unchanged
...
require_once(__DIR__ . "/lib/infra/BorhanLog.php");
...
$tmpXmlFileName = tempnam(sys_get_temp_dir(), 'borhan.generator.');
...
function showHelpAndExit()
{
	...
	echo "\t\t-r, --root:   \tRoot path, default is /opt/borhan.\n";
...
if(file_exists($outputPathBase))
{
	if(!$schemaXmlPath)
	{
		if(file_exists('BorhanClient.xml'))
			$schemaXmlPath = realpath('BorhanClient.xml');
		elseif(file_exists("$outputPathBase/BorhanClient.xml"))
			$schemaXmlPath = fixPath("$outputPathBase/BorhanClient.xml");
	}
}
else
{
...
BorhanLog::info("Downloading ready-made schema from: $schemaXmlPath");
...
BorhanLog::info("Generating from api version: $apiVersion, generated at: $generatedDate");
...
foreach($config as $name => $item)
{
	...
	BorhanLog::info("Now generating: $name using $generator");
	...
	else
	{
		...
		if($clearPath || file_exists($outputPath))
		{
			if (strtoupper(substr(PHP_OS, 0, 3)) === 'WIN')
			{
				...
				BorhanLog::info("Delete old files [$winOutputPath" . ($clearPath ? ", $clearPath" : "") . "]");
				passthru("rmdir /Q /S $winOutputPath $clearPath");
			}
			else
			{
				BorhanLog::info("Delete old files [$outputPath" . ($clearPath ? ", $clearPath" : "") . "]");
				passthru("rm -fr $outputPath $clearPath");
			}
		}
	}
		
	BorhanLog::info("Generate client library [$name]");
	$instance->setOutputPath($outputPath, $copyPath);
	$instance->setTestsPath($testsDir);
	$instance->generate();
	
	BorhanLog::info("Saving client library to [$outputPath]");
	...
	BorhanLog::info("$name generated successfully");
}
...
{
	BorhanLog::info("Trying to package");
	$output = shell_exec("tar --version");
	if ($output === null)
	{
		BorhanLog::warning("Skipping packaging, \"tar\" command not found! On Windows, tar can be installed using Cygwin, and it should be added to the path");
	}
	else
	{
		...
		BorhanLog::debug("Changing dir to [$outputPath]");
		chdir($outputPath);
		...
		if (file_exists($gzipOutputPath))
			BorhanLog::info("Package created successfully: $gzipOutputPath");
		else
			BorhanLog::err("Failed to create package");
			
		BorhanLog::debug("Restoring dir to [$oldDir]");
		chdir($oldDir);
	}
}
