<?php
/**
 * This is a port of Borhan's DotNetClientGenerator to Java.
 * 8/2009
 * jpotts, Optaros
 * 1/2010
 * oferc
...
class JavaClientGenerator extends ClientGeneratorFromXml 
{
	...
	protected $_baseClientPath = "src/main/java/com/borhan/client";
	...
	function writeEnum(DOMElement $enumNode) 
	{
		...
		$baseInterface = ($enumType == "string") ? "BorhanEnumAsString" : "BorhanEnumAsInt";
		
		$str = "";
		$str = "package com.borhan.client.enums;\n";
		$str .= "\n";
		$str .= $this->getBanner ();
	...
	function writeClass(DOMElement $classNode) 
	{
		...
		// Basic imports
		$imports = "";
		$imports .= "package com.borhan.client.types;\n\n";
		$imports .= "import org.w3c.dom.Element;\n";
		$imports .= "import com.borhan.client.BorhanParams;\n";
		$imports .= "import com.borhan.client.BorhanApiException;\n";
		...
		{
			$imports .= "import com.borhan.client.BorhanObjectBase;\n";
			$this->appendLine ( "public{$abstract} class $type extends BorhanObjectBase {" );
		}
	...
	public function generateParametersDeclaration(&$imports, $classNode) {
		...
		foreach($arrImportsEnums as $import) 
			$imports.= "import com.borhan.client.enums.$import;\n";
	...
	{	
		$type = $classNode->getAttribute ( "name" );
		$this->appendLine ( "    public BorhanParams toParams() throws BorhanApiException {" );
		$this->appendLine ( "        BorhanParams kparams = super.toParams();" );
	...
	{	
		$type = $classNode->getAttribute ( "name" );
		$this->appendLine ( "    public $type(Element node) throws BorhanApiException {" );
		$this->appendLine ( "        super(node);" );
			
		if ($classNode->childNodes->length) 
		{
			$imports .= "import com.borhan.client.utils.ParseUtils;\n";
...
//			$this->appendLine ( "            }" );
	...
	function writeService(DOMElement $serviceNode) 
	{
		...
		$imports = "";
		$imports .= "package com.borhan.client.services;\n\n";
		$imports .= "import com.borhan.client.BorhanClient;\n";
		$imports .= "import com.borhan.client.BorhanServiceBase;\n";
		$serviceName = $serviceNode->getAttribute ( "name" );
		
		$javaServiceName = $this->upperCaseFirstLetter ( $serviceName ) . "Service";
		$javaServiceType = "Borhan" . $javaServiceName;
		...
		$this->appendLine ( '@SuppressWarnings("serial")' );
		$this->appendLine ( "public class $javaServiceType extends BorhanServiceBase {" );
		$this->appendLine ( "    public $javaServiceType(BorhanClient client) {" );
		$this->appendLine ( "        this.borhanClient = client;" );
		$this->appendLine ( "    }" );
	...
	function writeAction($serviceId, DOMElement $actionNode, &$serviceImports) 
	{
		...
		$signature = $this->getSignature ( $paramNodesArr , array('' => 'BorhanFile'), $serviceImports);
		...
		if($desc)
			$this->appendLine ( $desc );
		$this->appendLine ( "    $signaturePrefix$signature throws BorhanApiException {" );
		
		$this->generateActionBodyServiceCall($serviceId, $action, $paramNodesArr, $serviceImports, $fallbackClass);
				
		if($resultType == 'file')
		{
			$this->appendLine ( "        return this.borhanClient.serve();");
		}
		else
		{
			...
			$this->appendLine ( "        if (this.borhanClient.isMultiRequest())" );
			$defaultValue = $this->getDefaultValue($resultType);
			$this->appendLine ( "            return $defaultValue;" );
						
			// Queue request
			if ($resultType)
				$this->appendLine ( "        Element resultXmlElement = this.borhanClient.doQueue();" );
			else 
				$this->appendLine ( "        this.borhanClient.doQueue();" );
		...
		$serviceImports[] = "com.borhan.client.BorhanParams";
		$serviceImports[] = "com.borhan.client.BorhanApiException";
	}
	...
	public function writeActionOverloads($signaturePrefix, $action, $resultType, $paramNodes, &$serviceImports)
	{
		...
		for($overloadNumber = 0; $overloadNumber < count ( $optionalParams ) + 1; $overloadNumber ++) 
		{
			...
			{
				if (reset($fileOverload) == 'BorhanFile' && $overloadNumber == count($optionalParams))
				...
				foreach ( $callParams as $paramNode ) 
				{
					...
					if ($paramType != "file" || reset($fileOverload) == 'BorhanFile')
					{
						$params[] = $paramName;
						continue;
					}
					...
					$params[] = "new BorhanFile(" . implode(', ', $fileParams) . ")";
				}				
	...
	public function generateActionBodyServiceCall($serviceId, $action, $paramNodes, &$serviceImports, $fallbackClass) 
	{
		...
		foreach ( $paramNodes as $paramNode )
		{
			...
			{
				$serviceImports[] = "com.borhan.client.BorhanFiles";
				$serviceImports[] = "com.borhan.client.BorhanFile";
				$haveFiles = true;
		...
		if ($haveFiles)
			if(is_null($fallbackClass))
				$this->appendLine ( "        this.borhanClient.queueServiceCall(\"$serviceId\", \"$action\", kparams, kfiles);" );
			else
				$this->appendLine ( "        this.borhanClient.queueServiceCall(\"$serviceId\", \"$action\", kparams, kfiles, $fallbackClass.class);" );
		else
			if(is_null($fallbackClass))
				$this->appendLine ( "        this.borhanClient.queueServiceCall(\"$serviceId\", \"$action\", kparams);" );
			else
				$this->appendLine ( "        this.borhanClient.queueServiceCall(\"$serviceId\", \"$action\", kparams, $fallbackClass.class);" );
	}
	
	public function handleResultType($resultType, $arrayType, &$serviceImports) 
	{
		$serviceImports[] = "com.borhan.client.utils.ParseUtils";
		$returnCall = "        ";
		switch ($resultType)
		{
			case "array" :
	...
	function writeMainClient(DOMNodeList $serviceNodes, DOMNodeList $configurationNodes) 
	{
		...
		$imports = "";
		$imports .= "package com.borhan.client;\n";
		
		$this->startNewTextBlock ();
		$this->appendLine ( $this->getBanner () );
		$this->appendLine ( '@SuppressWarnings("serial")' );
		$this->appendLine ( "public class BorhanClient extends BorhanClientBase {" );
		$this->appendLine ( "	" );
		$this->appendLine ( "	public BorhanClient(BorhanConfiguration config) {" );
		...
		foreach ( $serviceNodes as $serviceNode ) 
		{
			...
			$javaServiceType = "Borhan" . $this->upperCaseFirstLetter ( $javaServiceName );
			$imports .= "import com.borhan.client.services.$javaServiceType;\n";
		...
		foreach($configurationNodes as $configurationNode)
		{
			...
			foreach($configurationNode->childNodes as $configurationPropertyNode)
			{
				...
				{
					$imports .= "import com.borhan.client.types.$type;\n";
				}
		...
		$this->addFile ( $this->_baseClientPath . "/BorhanClient.java", $imports . $this->getTextBlock () );
	}
	...
	function getSignature($paramNodes, $fileOverload, &$serviceImports) 
	{
		...
		foreach ( $paramNodes as $paramNode ) 
		{
			...
			if ($paramType == "array")
			{
				...
				$serviceImports[] = "com.borhan.client.types.*";
			}	
			...
			elseif ($isEnum)
				$serviceImports[] = "com.borhan.client.enums.*";
			...
			if (strpos($paramType, 'Borhan') === 0 && !$isEnum)
				$serviceImports[] = "com.borhan.client.types.*";
	...
	private function getBanner() 
	{
		...
		$banner .= " * against an XML schema provided by Borhan.\n";
		$banner .= " * \n";
