...
class Xml2As3ClientGenerator extends ClientGeneratorFromXml
{
	...
	private function createTypeClass( $xml )
	{
		...
		$this->write2File( "com/borhan/types/" . $xml->attributes()->name . ".as" , $str );
	}
	...
	private function createVoClass( $xml )
	{
		...
		$str = "package com.borhan.vo\n";
		$str .= "{\n";

		if($xml->attributes()->base)
			$str .= "	import com.borhan.vo." . $xml->attributes()->base . ";\n\n";
		else if( $this->base_client_dir == "flex_client" )
		{
			$str .= "	import com.borhan.vo.BaseFlexVo;\n\n";
		}
		else //Must be flash client
		{
			$str .= "	import com.borhan.vo.BaseFlashVo;\n\n";
		}
		...
		foreach($xml->children() as $child)
		{
			...
			switch($child->attributes()->type)
			{
				...
				case "BorhanObjectBase" :
					$childType = "BaseFlexVo";
					break;
					
				default :
					$childType = $child->attributes()->type;
					$str = $this->addImport2String( "	import com.borhan.vo." . $childType , $str );
					break;
			}
			...
			if($this->generateDocs)
			{
				...
				{
					$str .= "		* @see com.borhan.types." . $child->attributes()->enumType . "\n";
				}
				elseif($child->attributes()->type == 'bool')
				{
					$str .= "		* @see com.borhan.types.borhanBoolean\n";
				}
				$str .= "		**/\n";
			}
		...
		$str .= "	}\n";
		$str .= "}\n";
		$this->write2File( "com/borhan/vo/$type.as" , $str );
	}
	...
	private function createCommands ( $xml )
	{
		...
		foreach($xml->children() as $child)
		{
				...
				if($prop->getName() == "param" )
				{
					switch($prop->attributes()->type)
					{
						...
						case "file" :
							...
							$imports .= "		import com.borhan.net.BorhanFileCall;\n";
						...
						default: //is Object
							...
							$const_props .= ",";
							$imports .= "		import com.borhan.vo." . $this->toUpperCamaleCase($prop->attributes()->type) . ";\n";
							if($prop->attributes()->optional == "1")
							{
								$keys_values_creator .= "			if (" . $prop->attributes()->name . ") { \n";
							}
							$keys_values_creator .= "				keyValArr = borhanObject2Arrays(" . $prop->attributes()->name . ", '" . $prop->attributes()->name . "');\n";
							$keys_values_creator .= "				keyArr = keyArr.concat(keyValArr[0]);\n";
							$keys_values_creator .= "				valueArr = valueArr.concat(keyValArr[1]);\n";
			...
			$str = "package com.borhan.commands.$type\n";
			$str .= "{\n";
			$str .= $imports;
			$str .= "	import com.borhan.delegates.$type." . $this->toUpperCamaleCase($type) . $this->toUpperCamaleCase( $child->attributes()->name ) . "Delegate;\n";

			if(!count($fileAttributesNames))
			$str .= "	import com.borhan.net.BorhanCall;\n";
			...
			if(count($fileAttributesNames))
			$str .= "extends BorhanFileCall\n" ;
			else
			$str .= "extends BorhanCall\n" ;
			...
			$this->write2File( "com/borhan/commands/$type/" . $this->toUpperCamaleCase($type) . $this->toUpperCamaleCase($child->attributes()->name) . ".as" , $str );
		}
	}
	...
	private function createServices( $xml )
	{
		...
		foreach($xml->children() as $child)
		{
			...
			$str = "package com.borhan.delegates." . $xml->attributes()->name . "\n";
			$str .= "{\n";
			$str .= "	import com.borhan.config.BorhanConfig;\n";
			$str .= "	import com.borhan.net.BorhanCall;\n";
			$str .= "	import com.borhan.delegates.WebDelegateBase;\n";
			if(count($fileAttributesNames))
			{
				$str .= "	import com.borhan.core.KClassFactory;\n";
				$str .= "	import com.borhan.errors.BorhanError;\n";
				$str .= "	import com.borhan.commands." . $xml->attributes()->name . "." . $this->toUpperCamaleCase($xml->attributes()->name) . $this->toUpperCamaleCase( $child->attributes()->name ) . ";\n\n";
			...
			$str .= "		public function " . $this->toUpperCamaleCase($xml->attributes()->name) . $this->toUpperCamaleCase( $child->attributes()->name ) . "Delegate(call:BorhanCall, config:BorhanConfig)\n";
			$str .= "		{\n";
			...
			switch( $child->result->attributes()->type )
			{
				...
				case "array":
					$str .= "		override public function parse(result:XML) : *\n";
					$str .= "		{\n";
					$str = $this->addImport2String( "	import com.borhan.core.KClassFactory" , $str );
					if( $child->result->attributes()->arrayType )
						$str = $this->addImport2String( "	import com.borhan.vo." . $child->result->attributes()->arrayType . ";" . $child->result->attributes()->arrayType . ";", $str);

					$str .= "			var arr : Array = new Array();\n";
					$str .= "			for( var i:int=0; i<result.result.children().length() ; i++)\n";
					$str .= "			{\n";
					$str .= "				var cls : Class = getDefinitionByName('com.borhan.vo.'+ result.result.children()[i].objectType) as Class;\n";
				...
				default:
					if(count($fileAttributesNames))
					{
						...
						$str .= "			}\n";
						$str .= "			else {\n";
						$str .= "				var cls : Class = getDefinitionByName('com.borhan.vo.'+ result.result.objectType) as Class;\n";
			...
			if(count($fileAttributesNames))
			{
				...
				$str .= "			}\n";
				$str .= " 			catch( e:Error ){\n";
				$str .= " 				var kErr : BorhanError = new BorhanError();\n";
			...
			$this->write2File( "com/borhan/delegates/" . $xml->attributes()->name . "/" . $this->toUpperCamaleCase($xml->attributes()->name) . $this->toUpperCamaleCase($child->attributes()->name) . "Delegate.as" , $str );
		}
	}
