...
class RubyClientGenerator extends ClientGeneratorFromXml
{
	...
	function generate() 
	{
		...
		$this->appendLine();
	  	$this->appendLine("end");
		$this->addFile("lib/borhan_enums.rb", $this->getTextBlock());
		
		
	
		// classes
		$this->startNewTextBlock();
		$this->appendLine("require 'borhan_enums.rb'");
		$this->appendLine();
		$this->appendLine("module Borhan");
		$this->appendLine();
		...
		$this->appendLine();
	  	$this->appendLine("end");
		$this->addFile("lib/borhan_types.rb", $this->getTextBlock());
		
		
		
		$this->startNewTextBlock();
		$this->appendLine("require 'borhan_client_base.rb'");
		$this->appendLine("require 'borhan_enums.rb'");
		$this->appendLine("require 'borhan_types.rb'");
		$this->appendLine();
		$this->appendLine("module Borhan");
		$this->appendLine();
		...
		$this->appendLine();
	  	$this->appendLine("end");
		$this->addFile("lib/borhan_client.rb", $this->getTextBlock());
		...
		foreach($pluginNodes as $pluginNode)
		{
			...
			$this->startNewTextBlock();
			$this->appendLine("require 'borhan_client.rb'");

			$dependencies = $pluginNode->getElementsByTagName("dependency");		
			foreach($dependencies as $dependency)
			{
				$this->appendLine("require File.dirname(__FILE__) + '/". $this->camelCaseToUnderscoreAndLower("Borhan".$this->upperCaseFirstLetter($dependency->getAttribute("pluginName"))."ClientPlugin.rb")."'");	
			}
			...
			$this->appendLine();
		  	$this->appendLine("end");
			$this->addFile($this->camelCaseToUnderscoreAndLower("lib/borhan_plugins/Borhan".$this->upperCaseFirstLetter($pluginName)."ClientPlugin.rb"), $this->getTextBlock());	
		}
	}
	...
	function writeClass(DOMElement $classNode)
	{
		...
		foreach($classNode->childNodes as $propertyNode)
		{
			...
			elseif($propType == 'array' || $propType == 'map')
			{
				...
				$this->appendLine("				self.".$this->camelCaseToUnderscoreAndLower($propName)." = BorhanClientBase.object_from_xml(xml_element.elements['$propName'], '$propArrayType')");	
			}
			else
			{
				$this->appendLine("				self.".$this->camelCaseToUnderscoreAndLower($propName)." = BorhanClientBase.object_from_xml(xml_element.elements['$propName'], '$propType')");	
			}
			$this->appendLine("			end");
		}	
		$this->appendLine("		end");
	...
	{
		$this->appendLine("	class BorhanClient < BorhanClientBase");
		foreach($serviceNodes as $serviceNode)
		{
			...
			$serviceClass = "Borhan".$this->upperCaseFirstLetter($serviceName)."Service";
	...
	protected function writeConfigurationProperty($configurationName, $name, $paramName)
	{
		...
		$this->appendLine("			end");
		$this->appendLine("			");
		$this->appendLine("			return BorhanNotImplemented");
		$this->appendLine("		end");
		$this->appendLine("		");
		$this->appendLine("	");
	}
