...
class TestmeDocGenerator extends ClientGeneratorFromXml
{
	...
	function generate()
	{
		...
		foreach($classNodes as $classNode)
		{
			...
			if(!$this->shouldIncludeType($type) || $type == 'BorhanObject')
				continue;
	...
	function appendHeader($cssPrefix = '../', $jquery = '')
	{
		...
		$this->appendLine('	<title>Borhan - TestMe Console Documentation</title>');
	...
	function writeEnum(DOMElement $enumNode)
	{
		...
		$this->appendLine('		<h2>Borhan API</h2>');
	...
	function writeClassBase($class, DOMElement $classNode)
	{
		...
		else 
		{
			...
			$this->appendLine('							<tr>');
			$this->appendLine('								<td class="inheritedFrom">');
			$this->appendLine('									<b><img class="base-attribute" src="../images/collapsed.gif" alt="BorhanObjectBase" /> Inherited from BorhanObjectBase</b>');
			$this->appendLine('								</td>');
			$this->appendLine('							</tr>');
			$this->appendLine('						</tbody>');
			$this->appendLine('					</table>');
			...
			$this->appendLine('								<td>map of <a href="BorhanListResponse.html">BorhanListResponse</a></td>');
	...
	function writeClassProperties($class, DOMElement $classNode)
	{
		...
		foreach($classNode->childNodes as $propertyNode)
		{
			...
			if($name == 'orderBy' && $this->endsWith($class, 'Filter') && $class != 'BorhanFilter')
			{
	...
	function writeAction($serviceId, DOMElement $actionNode)
	{
		...
		$this->appendLine('			</tr>');
		$this->appendLine('			<tr>');
		$this->appendLine("				<td colspan=\"5\">http://www.borhan.com/api_v3/service/$serviceId/action/$actionId<br />");
