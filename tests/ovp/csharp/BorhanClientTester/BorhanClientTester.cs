...
namespace Borhan
{
    class BorhanClientTester : IBorhanLogger
    {
        ...
        {
            BorhanClient client = new BorhanClient(GetConfig());
            client.KS = client.GenerateSession(ADMIN_SECRET, USER_ID, BorhanSessionType.ADMIN, PARTNER_ID, 86400, "");

            string fname = "DemoVideo.flv";
            FileStream fileStream = new FileStream(fname, FileMode.Open, FileAccess.Read, FileShare.Read);
            BorhanUploadToken myToken = new BorhanUploadToken();
            ...
            BorhanUploadToken uploadToken = client.UploadTokenService.Add(myToken);

            chunkThreaded(client.KS, fileStream, uploadToken.Id);
            
            BorhanUploadedFileTokenResource mediaResource = new BorhanUploadedFileTokenResource();
            mediaResource.Token = uploadToken.Id;
            BorhanMediaEntry mediaEntry = new BorhanMediaEntry();
        ...
        static void chunkThreaded(string ks, FileStream fileStream, string uploadTokenId)
        {
            ...
            BorhanUploadThread uploader = new BorhanUploadThread(ks, fileStream, chunkSize, ranges, uploadTokenId);
        ...
        public static BorhanConfiguration GetConfig()
        {
            BorhanConfiguration config = new BorhanConfiguration();
            config.ServiceUrl = SERVICE_URL;
            config.Logger = new BorhanClientTester();
            return config;
        }
        ...
        {
            BorhanClient client = new BorhanClient(GetConfig());
            client.KS = client.GenerateSession(ADMIN_SECRET, USER_ID, BorhanSessionType.ADMIN, PARTNER_ID);
			
            //verify that the account we're testing has the new iPad flavor enabled on the default conversion profile
            BorhanConversionProfile defaultProfile = client.ConversionProfileService.GetDefault();
            BorhanConversionProfileAssetParamsFilter  flavorsListFilter = new BorhanConversionProfileAssetParamsFilter();
            flavorsListFilter.SystemNameEqual = name;
			flavorsListFilter.ConversionProfileIdEqual = defaultProfile.Id;
			
            BorhanConversionProfileAssetParamsListResponse list = client.ConversionProfileAssetParamsService.List(flavorsListFilter);
        ...
        static void PlaylistExecuteMultiRequestExample()
        {
            ...
            // Request 1
            client.SessionService.Start(ADMIN_SECRET, "", BorhanSessionType.ADMIN, PARTNER_ID, 86400, "");
            ...
            BorhanMultiResponse response = client.DoMultiRequest();

            foreach (object obj in response)
            {
                if (obj.GetType() == typeof(BorhanAPIException))
                {
                    Console.WriteLine("Error occurred: " + ((BorhanAPIException)obj).Message);
                }
            }

            String twoEntries = "";

            if (response[1].GetType() == typeof(BorhanMediaListResponse))
            {
                BorhanMediaListResponse mediaListResponse = (BorhanMediaListResponse)response[1];
            ...
            string ks = client.GenerateSession(ADMIN_SECRET, USER_ID, BorhanSessionType.ADMIN, PARTNER_ID, 86400, "");
            client.KS = ks;

            BorhanPlaylist newPlaylist = new BorhanPlaylist();
            newPlaylist.Name = "Test Playlist";
            newPlaylist.PlaylistContent = twoEntries;
            newPlaylist.PlaylistType = BorhanPlaylistType.STATIC_LIST;

            BorhanPlaylist kPlaylist = client.PlaylistService.Add(newPlaylist);
            ...
            {
                if(currentResponse.GetType() != typeof(BorhanMultiResponse))
                {
        ...
        static void SampleReplaceVideoFlavorAndAddCaption()
        {
            ...
            BorhanClient client = new BorhanClient(GetConfig());
            client.KS = client.GenerateSession(ADMIN_SECRET, USER_ID, BorhanSessionType.ADMIN, PARTNER_ID, 86400, "");
            BorhanUploadToken uploadToken = client.UploadTokenService.Add();
            ...
            else
            {
                ...
                BorhanFlavorAsset iPadFlavor = null;
                while (statusB == false)
                {
                    Console.WriteLine("2. Waiting for the iPad flavor to be available...");
                    System.Threading.Thread.Sleep(5000);
                    BorhanFlavorAssetFilter flavorAssetsFilter = new BorhanFlavorAssetFilter();
                    flavorAssetsFilter.EntryIdEqual = mediaEntry.Id;
                    BorhanFlavorAssetListResponse flavorAssets = client.FlavorAssetService.List(flavorAssetsFilter);
                    foreach (BorhanFlavorAsset flavor in flavorAssets.Objects)
                    {
                        if (flavor.FlavorParamsId == iPadFlavorId)
                        {
                            iPadFlavor = flavor;
                            statusB = flavor.Status == BorhanFlavorAssetStatus.READY;
                            if (flavor.Status == BorhanFlavorAssetStatus.NOT_APPLICABLE)
                            {
                                //in case the Borhan Transcoding Decision Layer decided not to convert to this flavor, let's force it.
                ...
                string playManifestURL = "http://www.borhan.com/p/{partnerId}/sp/0/playManifest/entryId/{entryId}/format/url/flavorParamId/{flavorParamId}/ks/{ks}/{fileName}.mp4";
                ...
                //now let's replace the flavor with our video file (e.g. after processing the file outside of Borhan)
                ...
                mediaResource = new BorhanUploadedFileTokenResource();
                mediaResource.Token = uploadToken.Id;
                BorhanFlavorAsset newiPadFlavor = client.FlavorAssetService.SetContent(iPadFlavor.Id, mediaResource);
            ...
            BorhanCaptionAsset captionAsset = new BorhanCaptionAsset();
            captionAsset.Label = "Test C# Uploaded Caption";
            captionAsset.Language = BorhanLanguage.EN;
            captionAsset.Format = BorhanCaptionType.SRT;
            ...
            BorhanUploadedFileTokenResource captionResource = new BorhanUploadedFileTokenResource();
        ...
        static void SampleMetadataOperations()
        {
            ...
            BorhanMediaEntryFilter mediaEntryFilter = new BorhanMediaEntryFilter();
            mediaEntryFilter.OrderBy = BorhanMediaEntryOrderBy.CREATED_AT_ASC;
            mediaEntryFilter.MediaTypeEqual = BorhanMediaType.VIDEO;

            BorhanFilterPager pager = new BorhanFilterPager();
            pager.PageSize = 1;
            pager.PageIndex = 1;

            BorhanMetadataProfile newMetadataProfile = new BorhanMetadataProfile();
            newMetadataProfile.MetadataObjectType = BorhanMetadataObjectType.ENTRY;
            newMetadataProfile.Name = "Test";

            Console.WriteLine("List videos, get the first one...");
            IList<BorhanMediaEntry> entries = client.MediaService.List(mediaEntryFilter, pager).Objects;
            BorhanMediaEntry entry = entries[0];
            
            BorhanMetadataProfile metadataProfile = client.MetadataProfileService.Add(newMetadataProfile, xsd);
            Console.WriteLine("1. Successfully created the custom metadata profile " + metadataProfile.Name + ".");

            BorhanMetadata metadata = client.MetadataService.Add(metadataProfile.Id, metadataProfile.MetadataObjectType, entry.Id, xmlData);
            ...
            IList<BorhanMetadata> metadataList = client.MetadataService.List(metadataFilter).Objects;
        ...
        static BorhanMediaEntry StartSessionAndUploadMedia(FileStream fileStream)
        {
            BorhanClient client = new BorhanClient(GetConfig());

            // start new session (client session is enough when we do operations in a users scope)
            client.KS = client.GenerateSession(ADMIN_SECRET, USER_ID, BorhanSessionType.USER, PARTNER_ID, 86400, "");
        ...
        private static void AdvancedMultiRequestExample()
        {
            ...
            client.KS = client.GenerateSession(ADMIN_SECRET, "", BorhanSessionType.ADMIN, PARTNER_ID);
            client.StartMultiRequest();
            ...
            if (response[3] is BorhanMediaEntry)
            {
                BorhanMediaEntry newMediaEntry = (BorhanMediaEntry)response[3];
                Console.WriteLine("Multirequest newly added entry id: " + newMediaEntry.Id + ", status: " + newMediaEntry.Status);
            }
        }
        
	    private static BorhanMediaEntry createEntry()
	    {
            BorhanClient client = new BorhanClient(GetConfig());
            client.KS = client.GenerateSession(ADMIN_SECRET, USER_ID, BorhanSessionType.USER, PARTNER_ID, 86400, "");
	    ...
	    private static BorhanMetadata createMetadata(int metadataProfileId, BorhanMetadataObjectType objectType, string objectId, string xmlData)
	    {
            BorhanClient client = new BorhanClient(GetConfig());
            client.KS = client.GenerateSession(ADMIN_SECRET, USER_ID, BorhanSessionType.USER, PARTNER_ID, 86400, "");
	    ...
	    private static BorhanMetadataProfile createMetadataProfile(BorhanMetadataObjectType objectType, string xsdData)
	    {
            BorhanClient client = new BorhanClient(GetConfig());
            client.KS = client.GenerateSession(ADMIN_SECRET, USER_ID, BorhanSessionType.ADMIN, PARTNER_ID, 86400, "");
        ...
        private static IList<BorhanMediaEntry> createEntriesWithMetadataObjects(int entriesCount)
	    {
            return createEntriesWithMetadataObjects(entriesCount, 2);
        }

        private static IList<BorhanMediaEntry> createEntriesWithMetadataObjects(int entriesCount, int metadataProfileCount)
	    {
		    IList<BorhanMediaEntry> entries = new List<BorhanMediaEntry>(2);
            IDictionary<string, BorhanMetadataProfile> metadataProfiles = new Dictionary<string, BorhanMetadataProfile>(3);
        ...
        <xsd:simpleType name=""listType"">
            <xsd:restriction base=""xsd:string"" />
                ...
                metadataProfiles.Add(i.ToString(), createMetadataProfile(BorhanMetadataObjectType.ENTRY, xsd));
            }
    		
            string xml;
            for(int i = 0; i < entriesCount; i++)
            {
                BorhanMediaEntry entry = createEntry();
                entries.Add(entry);
        ...
        <FreeText" + index + ">example text " + index + "</FreeText" + index + @">
                    ...
                    createMetadata(metadataProfiles[index].Id, BorhanMetadataObjectType.ENTRY, entry.Id, xml);
                }
            }
        ...
        private static void ResponseProfileExample()
        {
            ...
            IList<BorhanMediaEntry> entries = createEntriesWithMetadataObjects(entriesTotalCount, metadataProfileTotalCount);
    		
            BorhanMediaEntryFilter entriesFilter = new BorhanMediaEntryFilter();
            entriesFilter.StatusIn = BorhanEntryStatus.PENDING.ToString() + "," + BorhanEntryStatus.NO_CONTENT.ToString();
            entriesFilter.TagsLike = uniqueTag;
    		
            BorhanFilterPager entriesPager = new BorhanFilterPager();
            ...
            IList<BorhanResponseProfileMapping> metadataMappings = new List<BorhanResponseProfileMapping>();
            metadataMappings.Add(metadataMapping);
    		
            BorhanFilterPager metadataPager = new BorhanFilterPager();
            ...
            IList<BorhanDetachedResponseProfile> metadataResponseProfiles = new List<BorhanDetachedResponseProfile>();
            metadataResponseProfiles.Add(metadataResponseProfile);

            BorhanResponseProfile responseProfile = new BorhanResponseProfile();
            ...
            BorhanResponseProfileHolder nestedResponseProfile = new BorhanResponseProfileHolder();
            nestedResponseProfile.Id = responseProfile.Id;
    		
            client.ResponseProfile = nestedResponseProfile;
            IList<BorhanBaseEntry> list = client.BaseEntryService.List(entriesFilter, entriesPager).Objects;
            ...
            foreach(BorhanBaseEntry entry in list)
            {	
                ...
                if (!(entry.RelatedObjects[metadataResponseProfile.Name] is BorhanMetadataListResponse))
                {
                ...
                BorhanMetadataListResponse metadataListResponse = (BorhanMetadataListResponse)entry.RelatedObjects[metadataResponseProfile.Name];
                ...
                foreach(BorhanMetadata metadata in metadataListResponse.Objects)
                {
