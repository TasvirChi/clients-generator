...
class ZendClientTester
{
	...
	public function __construct(Borhan_Client_Client $client, $partnerId)
	{
		$this->_client = $client;
	...
	public function run()
	{
		...
		$mixEntry = new Borhan_Client_Type_MixEntry();
		$mixEntry->name = ".Net Mix";
		$mixEntry->editorType = Borhan_Client_Enum_EditorType::SIMPLE;
		...
		$mediaEntry = new Borhan_Client_Type_MediaEntry();
		$mediaEntry->name = "Media Entry For Mix";
		$mediaEntry->mediaType = Borhan_Client_Enum_MediaType::VIDEO;
		...
		foreach( $response as $subResponse)
			if($subResponse instanceof Borhan_Client_Exception) 
		...
		$this->assertTrue($response[0] instanceof Borhan_Client_Type_MixEntry);
		$mixEntry = $response[0];
	...
	{
		// add upload token
		$uploadToken = new Borhan_Client_Type_UploadToken();
    	...
    	// count media entries
    	$mediaFilter = new Borhan_Client_Type_MediaEntryFilter();
    	$mediaFilter->idEqual = $entry->id;
    	$mediaFilter->statusNotEqual = Borhan_Client_Enum_EntryStatus::DELETED;
	...
	public function testReturnedArrayObjectUsingPlaylistExecute()
	{
    	...
    	$playlistFilter = new Borhan_Client_Type_MediaEntryFilterForPlaylist();
	...
	public function testServeUrl()
	{
		...
		$thumbAssetFilter = new Borhan_Client_Type_ThumbAssetFilter();
	...
	public function addImageEntry()
	{
		$entry = new Borhan_Client_Type_MediaEntry();
    	$entry->name = self::ENTRY_NAME;
    	$entry->mediaType = Borhan_Client_Enum_MediaType::IMAGE;
