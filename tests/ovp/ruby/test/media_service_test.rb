...
class MediaServiceTest < Test::Unit::TestCase
  ...
  # this test uploads a video file to borhan and creates a media entry using the uploaded file.
  should "upload a video file and create a borhan entry" do
        
    media_entry = Borhan::BorhanMediaEntry.new
    media_entry.name = "borhan_test1"
    media_entry.media_type = Borhan::BorhanMediaType::VIDEO
  ...
  # this test uploads a image file to borhan and creates a media entry using the uploaded file.  
  should "upload a image file and create a borhan entry" do
    
    media_entry = Borhan::BorhanMediaEntry.new
    media_entry.name = "borhan_test2"
    media_entry.media_type = Borhan::BorhanMediaType::IMAGE
  ...
  # this test uploads a video file to borhan reading from a url and creates a media entry using the uploaded file.  
  should "upload a flv file from url and create a borhan entry" do
    
    media_entry = Borhan::BorhanMediaEntry.new
    media_entry.name = "borhan_test3"
    media_entry.media_type = Borhan::BorhanMediaType::VIDEO
    
    created_entry3 = @client.media_service.add_from_url(media_entry, URI.escape("http://www.borhan.org/kalorg/borhan-api-clients-generators/trunk/sources/csharp/BorhanClientTester/DemoVideo.flv", Regexp.new("[^#{URI::PATTERN::UNRESERVED}]")))
    
    assert_not_nil created_entry3.id
    assert_equal created_entry3.status, Borhan::BorhanEntryStatus::IMPORT
  ...
  should "update the thumbnail from url" do
    ...
    updated_entry = @client.media_service.update_thumbnail_from_url(created_entry.id, "http://corp.borhan.com/sites/borhan-website/files/logo_0.png")
  ...
  should "not raise any errors for double start multi request" do
    ...
    assert_nothing_raised Borhan::BorhanAPIError do
        @client.start_multirequest
        @client.start_multirequest
    end
  end
  ...
  should "get an empty array for the multirequest when it calls without starting it" do
    ...
    media_entry_filter = Borhan::BorhanMediaEntryFilter.new
    media_entry_filter.name_multi_like_or = "borhan_test3"
  ...
  should "get the media list and create a media entry" do
    ...
    assert_instance_of Borhan::BorhanMediaEntry, retVal[1]
    assert_instance_of Borhan::BorhanMediaListResponse, retVal[2]    
  ...
  should "return error objects for failed actions" do
    ...
    assert_instance_of Borhan::BorhanAPIError, retVal[0]
    assert_instance_of Borhan::BorhanMediaListResponse, retVal[1]
  ...
  should "support setting nil for content in entry metadata" do
    ...
    media_entry = Borhan::BorhanMediaEntry.new
    media_entry.name = "borhan_test1"
    media_entry.description = "borhan_test1 description"
    media_entry.media_type = Borhan::BorhanMediaType::VIDEO
    video_file = File.open("test/media/test.wmv")
    ...
    assert_equal media_entry.name, "borhan_test1"
    assert_equal media_entry.description, "borhan_test1 description"
    
    # update the desc with value
    media_entry = Borhan::BorhanBaseEntry.new
    media_entry.name = "borhan_test1 updated"
