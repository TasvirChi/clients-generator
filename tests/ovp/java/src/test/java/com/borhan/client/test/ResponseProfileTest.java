...
public class ResponseProfileTest extends BaseTest{
	...
	public void testEntryCategoriesAndMetadata() throws Exception {
		...
		try{
			...
			metadataFilter.metadataObjectTypeEqual = BorhanMetadataObjectType.CATEGORY;
			...
			BorhanCategoryEntry categoryEntry = addEntryToCategory(entry.id, category.id);
			BorhanMetadata categoryMetadata = createMetadata(BorhanMetadataObjectType.CATEGORY, Integer.toString(category.id), categoryMetadataProfile.id, xml);
			...
			BorhanMediaEntry getEntry = client.getMediaService().get(entry.id);
			...
			BorhanCategoryEntryListResponse categoryEntryList = (BorhanCategoryEntryListResponse) getEntry.relatedObjects.get(categoryEntryResponseProfile.name);
			assertEquals(1, categoryEntryList.totalCount);
			BorhanCategoryEntry getCategoryEntry = categoryEntryList.objects.get(0);
			...
			BorhanMetadataListResponse metadataList = (BorhanMetadataListResponse) getCategoryEntry.relatedObjects.get(metadataResponseProfile.name);
			assertEquals(1, metadataList.totalCount);
			BorhanMetadata getMetadata = metadataList.objects.get(0);
