...
function runJavaTests($clientRoot)
{
	...
	executeCommand("{$jdkPath}javac.exe", "-d . -sourcepath ../src -cp ".implode(';', addPrefix($externalJars, '../lib/'))." ../src/com/borhan/client/test/BorhanTestSuite.java");

	// pack the client library
	executeCommand("{$jdkPath}jar.exe", "cvf borhanClient.jar .");
	...
	$jarList = "bin/borhanClient.jar;".implode(';', addPrefix($externalJars, 'lib/'));
	
	chdir($clientRoot);
	executeCommand("{$jdkPath}java.exe", "-cp {$jarList} {$log4jParam} org.junit.runner.JUnitCore com.borhan.client.test.BorhanTestSuite");
}
...
function runCSharpTests($clientRoot)
{
	...
	// clean up
	$exeFile = fixSlashes("{$clientRoot}/BorhanClientTester/bin/Debug/BorhanClientTester.exe");
	if (file_exists($exeFile))
		unlink($exeFile);
	
	// compile
	executeCommandFrom($clientRoot, $config['csharp']['devenv_bin'], "/build Debug BorhanClient.sln");
	...
	// run the tests
	executeCommandFrom("{$clientRoot}/BorhanClientTester", $exeFile);
}
...
executeCommandFrom("{$rootDir}/python/BorhanClient/tests", $config['python']['python_bin'], '-m unittest discover');
...
echo "Flex3.5\n==================\n";
executeCommandFrom("{$rootDir}/flex35", $config['flex35']['mxmlc_bin'], "-sp tests . -- tests/BorhanClientSample.as");
